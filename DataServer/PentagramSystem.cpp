...
void CPentagramSystem::GDPentagramJewelDeleteSaveRecv(SDHP_PENTAGRAM_JEWEL_DELETE_SAVE_RECV* lpMsg) // OK
{
	...
	for (int n = 0; n < MAX_CHARACTERS; n++ )
	{
		...
		gQueryManager.ExecQuery("DELETE FROM PentagramJewel WHERE Name='%s' AND Type=%d AND [Index]=%d",CharacterName[n],lpMsg->Type,lpMsg->Index);
		gQueryManager.Close();
	}
}
