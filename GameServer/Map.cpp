...
bool CMap::LabyrinthMoneyItemDrop(int aIndex, int money, int x, int y) // OK
{
	if (this->CheckAttr(x, y, 4) != 0 || this->CheckAttr(x, y, 8) != 0)
	{
		return 0;
	...
	for (int n = 0; n < MAX_MAP_ITEM; n++)
	{
		if (this->m_Item[count].IsItem() == 0)
		{
#if(GAMESERVER_UPDATE>=1200)//2018/01/14
			if (gObj[aIndex].Map == MAP_Labyrinth)
			{
				this->m_Item[count].m_UserIndex = aIndex;
			}
#endif
#if(GAMESERVER_UPDATE>=1220)//2018/05/08
			if (gObj[aIndex].Map == MAP_4_TH_QUEST)
			{
				this->m_Item[count].m_UserIndex = aIndex;
			}
#endif
...
bool CMap::CheckItemGive(int aIndex,int index) // OK
{
	...
	//if (lpObj->m_cLabyrinthInfo->IsPlaying()==0)
	//{
...
int CMap::ReLoadMapAttr(const char *szFileName, WORD wMapNumber)
{
	HANDLE file = CreateFile(szFileName, GENERIC_READ, FILE_SHARE_READ, 0, OPEN_EXISTING, FILE_ATTRIBUTE_ARCHIVE, 0);

	if (file == INVALID_HANDLE_VALUE)
	{
		ErrorMessageBox(MEM_SCRIPT_ERROR_CODE0, szFileName);
		return -1;
	}
	...
	if (ReadFile(file, &head, sizeof(head), &BytesRead, 0) == 0)
	{
		ErrorMessageBox(MEM_SCRIPT_ERROR_CODE2, szFileName);
		CloseHandle(file);
		return -1;
	}
	...
	if (width < 0 || width >= MAX_MAP_WIDTH || height < 0 || height >= MAX_MAP_HEIGHT)
	{
		ErrorMessageBox(MEM_SCRIPT_ERROR_CODE4, szFileName);
		CloseHandle(file);
		LogAdd(LOG_BLACK,"[ERROR-L1] Reload map attr over value width, height.(w:%d)(h:%d)", this->m_width, this->m_height);
		return -1;
	}
	...
	if (this->m_MapAttr == 0)
	{
		ErrorMessageBox(MEM_SCRIPT_ERROR_CODE1, szFileName);
		CloseHandle(file);
		LogAdd(LOG_BLACK, "[ERROR-L1] Reload map attr, m_attrbuf is null.");
		return -1;
	}
