...
void CDoubleGoer::SetState_START() // OK
{
	...
	//LogAdd(LOG_DEBUG, "EVENTO COMEÇOU");
...
void CDoubleGoer::CGDoubleGoerEnterRecv(PMSG_DOUBLE_GOER_ENTER_RECV* lpMsg,int aIndex) // OK
{
	...
	if (leveldec >= 50 && leveldec <= 179)
	{
		level = 0;
	}
	else if (leveldec >= 180 && leveldec <= 269)
	{
		level = 1;
	}
	else if (leveldec >= 270 && leveldec <= 349)
	{
		level = 2;
	}
	else if (leveldec >= 350 && leveldec <= 399)
	{
		level = 3;
	}
	else if (leveldec >= 400 && leveldec <= 500)
	{
		level = 4;
	}
	else if (leveldec >= 501 && leveldec <= 650)
	{
		level = 5;
	}
	else if (leveldec >= 651)
	{
		level = 6;
	}

	this->m_Map = MAP_DOUBLE_GOER1;
	...
	if(OBJECT_RANGE(this->m_PartyNumber) == 0)
	{
		if(this->AddUser(aIndex) != 0)
		{
			...
			gObjMoveGate(aIndex, 345 + this->m_Level);
