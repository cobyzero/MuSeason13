...
#if(GAMESERVER_UPDATE>=1000)//2017/05/06
...
int CSocketItemOption::GetSocketItemExtraMoney(CItem* lpItem) // OK
{
	...
	for (int n = 0; n < MAX_SOCKET_ITEM_OPTION_TABLE; n++)
	{
		...
		{
#if(GAMESERVER_UPDATE>=1120)//2018/04/09
			int ItemOptionLevel;
			int ItemOptionType;
...
CItem CSocketItemOption::GetSocketExtractSeedSphere(CItem* lpItem, int type)
{
	...
	for (int n = 0; n < MAX_SOCKET_ITEM_OPTION; n++)
	{
		for (int i = 0; i < 10; i++)
		{
			...
			if ((lpInfo->Index + (i * MAX_SOCKET_ITEM_OPTION)) == (lpItem->m_SocketOption[type] + 0x100 - 2))
			{
				item.m_Level = lpInfo->SubIndex;

				if (i < 5)
					item.m_Index = GET_ITEM(12, 100) + i * 6 + (lpInfo->Type - 1);
				else
					item.m_Index = GET_ITEM(12, 311) + (i - 5) * 6 + (lpInfo->Type - 1);
...
BYTE CSocketItemOption::GetSocketItemBonusOption(CItem* lpItem) // OK
{
	for (int n = 0; n < MAX_SOCKET_ITEM_OPTION; n++)
	{
		...
		{
			if (lpItem->m_SocketOption[0] == lpItem->m_SocketOption[1] == lpItem->m_SocketOption[2])
			{
	...
	for (int n = 0; n < MAX_SOCKET_ITEM_OPTION_TABLE; n++)
	{
		if (lpItem->m_SocketOption[n] != SOCKET_ITEM_OPTION_NONE && lpItem->m_SocketOption[n] != SOCKET_ITEM_OPTION_EMPTY)
		{
			...
			if (lpItem->m_NewOption != 0 || lpItem->m_SetOption != 0)
			{
				...
				ItemOption = socketOption % MAX_SOCKET_ITEM_OPTION;
			}
			else
			{
				ItemOption = lpItem->m_SocketOption[n] % MAX_SOCKET_ITEM_OPTION;
			}
	...
	for (int n = 0; n < INVENTORY_WEAR_SIZE; n++)
	{
		...
		for (int i = 0; i < MAX_SOCKET_ITEM_OPTION_TABLE; i++)
		{
			if (lpObj->Inventory[n].m_SocketOption[i] != SOCKET_ITEM_OPTION_NONE && lpObj->Inventory[n].m_SocketOption[i] != SOCKET_ITEM_OPTION_EMPTY)
			{
				...
				if (lpObj->Inventory[n].m_NewOption != 0 || lpObj->Inventory[n].m_SetOption != 0)
				{
					int socketOption = lpObj->Inventory[n].m_SocketOption[i] + 0x100;
				...
				{
					ItemOption = lpObj->Inventory[n].m_SocketOption[i] % MAX_SOCKET_ITEM_OPTION;
					ItemOptionLevel = lpObj->Inventory[n].m_SocketOption[i] / MAX_SOCKET_ITEM_OPTION;
				}
...
void CSocketItemOption::InsertOption(LPOBJ lpObj, int index, int value, bool flag) // OK
{
...
#if(GAMESERVER_UPDATE>=1120)//2017/05/06
		lpObj->PhysiDamageMinLeft += (value == 0 ? 0 : ((lpObj->Level / 20) * value));
		lpObj->PhysiDamageMinRight += (value == 0 ? 0 : ((lpObj->Level / 20) * value));
		lpObj->PhysiDamageMaxLeft += (value == 0 ? 0 : ((lpObj->Level / 20) * value));
		lpObj->PhysiDamageMaxRight += (value == 0 ? 0 : ((lpObj->Level / 20) * value));
		lpObj->MagicDamageMin += (value == 0 ? 0 : ((lpObj->Level / 20) * value));
		lpObj->MagicDamageMax += (value == 0 ? 0 : ((lpObj->Level / 20) * value));
		lpObj->CurseDamageMin += (value == 0 ? 0 : ((lpObj->Level / 20) * value));
		lpObj->CurseDamageMax += (value == 0 ? 0 : ((lpObj->Level / 20) * value));
#else
	...
	case SOCKET_ITEM_OPTION_ADD_SHIELD_DAMAGE_REDUCTION:
		//#if(GAMESERVER_UPDATE>=1120)//2017/05/06
					//lpObj->ShieldDamageReduction += (lpObj->ShieldDamageReduction/100)*value;
		//#else
