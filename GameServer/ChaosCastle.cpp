...
void CChaosCastle::GiveUserRewardItem(CHAOS_CASTLE_LEVEL* lpLevel,int aIndex) // OK
{
	...
	{
		case 0:
			gItemBagManager.GremoryCaseDropItemBySpecialValue(ITEM_BAG_CHAOS_CASTLE1,GC_STORAGE_CHARACTER,GC_REWARD_CHAOS_CASTLE,&gObj[lpUser->Index],gObj[lpUser->Index].Map,gObj[lpUser->Index].X,gObj[lpUser->Index].Y);
			break;
		case 1:
			gItemBagManager.GremoryCaseDropItemBySpecialValue(ITEM_BAG_CHAOS_CASTLE2,GC_STORAGE_CHARACTER,GC_REWARD_CHAOS_CASTLE,&gObj[lpUser->Index],gObj[lpUser->Index].Map,gObj[lpUser->Index].X,gObj[lpUser->Index].Y);
			break;
		case 2:
			gItemBagManager.GremoryCaseDropItemBySpecialValue(ITEM_BAG_CHAOS_CASTLE3,GC_STORAGE_CHARACTER,GC_REWARD_CHAOS_CASTLE,&gObj[lpUser->Index],gObj[lpUser->Index].Map,gObj[lpUser->Index].X,gObj[lpUser->Index].Y);
			break;
		case 3:
			gItemBagManager.GremoryCaseDropItemBySpecialValue(ITEM_BAG_CHAOS_CASTLE4,GC_STORAGE_CHARACTER,GC_REWARD_CHAOS_CASTLE,&gObj[lpUser->Index],gObj[lpUser->Index].Map,gObj[lpUser->Index].X,gObj[lpUser->Index].Y);
			break;
		case 4:
			gItemBagManager.GremoryCaseDropItemBySpecialValue(ITEM_BAG_CHAOS_CASTLE5,GC_STORAGE_CHARACTER,GC_REWARD_CHAOS_CASTLE,&gObj[lpUser->Index],gObj[lpUser->Index].Map,gObj[lpUser->Index].X,gObj[lpUser->Index].Y);
			break;
		case 5:
			gItemBagManager.GremoryCaseDropItemBySpecialValue(ITEM_BAG_CHAOS_CASTLE6,GC_STORAGE_CHARACTER,GC_REWARD_CHAOS_CASTLE,&gObj[lpUser->Index],gObj[lpUser->Index].Map,gObj[lpUser->Index].X,gObj[lpUser->Index].Y);
			break;
		case 6:
			gItemBagManager.GremoryCaseDropItemBySpecialValue(ITEM_BAG_CHAOS_CASTLE7,GC_STORAGE_CHARACTER,GC_REWARD_CHAOS_CASTLE,&gObj[lpUser->Index],gObj[lpUser->Index].Map,gObj[lpUser->Index].X,gObj[lpUser->Index].Y);
			break;
	}
}
...
void CChaosCastle::CGChaosCastleEnterRecv(PMSG_CHAOS_CASTLE_ENTER_RECV* lpMsg,int aIndex) // OK
{
	...
	{
		GCMessagePopupSend(lpObj, "Wrong Chaos Castle level!");
		return;
	}
	...
	{
		GCMessagePopupSend(lpObj, "Invalid ticket item position!");
		return;
	}

	if(lpObj->Inventory[lpMsg->slot].m_Index != GET_ITEM(13, 29) && lpObj->Inventory[lpMsg->slot].m_Index != GET_ITEM(13, 121))
	{
		GCMessagePopupSend(lpObj, "Invalid ticket item!");
		return;
	}

	if (gEffectManager.CheckEffect(lpObj, EFFECT_USE_MOUNT_UNIRIA) || gEffectManager.CheckEffect(lpObj, EFFECT_USE_MOUNT_DINORANT) || gEffectManager.CheckEffect(lpObj, EFFECT_USE_MOUNT_FENRIR))
	{
		GCMessagePopupSend(lpObj, "You can't enter Chaos Castle using a Mount!");
		return;
	}
	...
	{
		GCMessagePopupSend(lpObj, "You can't enter Chaos Castle from the current map!");
		return;
	}

	if(OBJECT_RANGE(lpObj->PartyNumber) != 0)
	{
		GCMessagePopupSend(lpObj, "You can't enter Chaos Castle being in a Party!");
		return;
	}

	if(lpObj->Guild != 0 && lpObj->Guild->WarState == 1)
	{
		GCMessagePopupSend(lpObj, "You can't enter Chaos Castle while in a Guild War!");
		return;
	}

	if(OBJECT_RANGE(lpObj->DuelUser) != 0)
	{
		GCMessagePopupSend(lpObj, "You can't enter Chaos Castle while in a Duel!");
		return;
	}

	if(lpObj->PShopOpen != 0)
	{
		GCMessagePopupSend(lpObj, "You can't enter Chaos Castle with your Personal Shop opened!");
		return;
	}

	if(gObjIsChangeSkin(lpObj->Index) != 0) //testar skin //debugar mensagens de erro com comando
	{
		GCMessagePopupSend(lpObj, "You can't enter Chaos Castle using a transformation ring or skin!");
		return;
	}

	if(this->GetEnterEnabled(lpMsg->level) == 0)
	{
		GCMessagePopupSend(lpObj, "This Chaos Castle level is not available at the moment!");
		return;
	}

	if (lpObj->Money < ((DWORD)gChaosCastleEnterMoney[lpMsg->level]))
	{
		GCMessagePopupSend(lpObj, "You have not enough money to enter Chaos Castle!");
		return;
	}

	if (gServerInfo.m_PKLimitFree == 0 && lpObj->PKLevel >= 4)
	{
		GCMessagePopupSend(lpObj, "You can't enter Chaos Castle with your PK status!");
		return;
	}

	if(gMapServerManager.CheckMapServer(lpLevel->Map) == 0)
	{
		GCMessagePopupSend(lpObj, "Please go to the Event Square map before entering Chaos Castle!");
		return;
	}	

	if(this->AddUser(lpLevel,aIndex) == 0)
	{
		GCMessagePopupSend(lpObj, "You can't join Chaos Castle now!");
		return;
	}
	...
	else if (lpObj->Inventory[lpMsg->slot].m_Index == GET_ITEM(13, 121))
	{
