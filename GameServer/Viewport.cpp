...
void CViewport::GCViewportPlayerSend(int aIndex) // OK
{
	...
	for (int n = 0; n < MAX_VIEWPORT; n++)
	{
		...
		if(!gFourthQuest.ShouldViewPlayer(lpObj, lpTarget)) continue;
		...
		/*ZeroMemory(info.MuunItem1, sizeof(info.MuunItem1));
		ZeroMemory(info.MuunItem2, sizeof(info.MuunItem2));
		ZeroMemory(info.MuunItem3, sizeof(info.MuunItem3));*/
		...
		//LogAdd(LOG_GREEN, "[%s] DarkSpirit 1: %d %d %d", lpTarget->Name, info.DarkSpirit, lpTarget->m_btInvenPetDkSpiritPos, lpTarget->Inventory[lpTarget->m_btInvenPetDkSpiritPos].m_JewelOfHarmonyOption);
...
void CViewport::GCViewportMonsterSend(int aIndex) // OK
{
	...
	for (int n = 0; n < MAX_VIEWPORT; n++)
	{
		...
		if(!gFourthQuest.ShouldViewMonster(lpObj, lpTarget)) continue;
...
void CViewport::GCViewportSimplePlayerSend(LPOBJ lpObj) // OK
{
	...
	//LogAdd(LOG_RED, "Muun: %d %d %d", lpObj->MuunInventory[0].m_Index, lpObj->MuunInventory[1].m_Index, lpObj->MuunInventory[2].m_Index);
	...
	//LogAdd(LOG_BLUE, "[%s] DarkSpirit Simple: %d %d %d", lpObj->Name, info.DarkSpirit, lpObj->m_btInvenPetDkSpiritPos, lpObj->Inventory[lpObj->m_btInvenPetDkSpiritPos].m_JewelOfHarmonyOption);
