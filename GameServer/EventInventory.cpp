...
bool CEventInventory::IsEventItem(int ItemIndex) // OK
{
	...
	//if(gItemBagManager.GetItemByLocation(ItemIndex))
	if(gGItemSlot.GetItemBySlot(ItemIndex,1))
	{
		return 1;
	}
	else
...
{
	#if(GAMESERVER_UPDATE>=1200)//2017/06/02
	if (gGItemSlot.GetItemBySlot(lpItem->m_Index, 1))
	{
		return 1;
	}
	else
	...
	if(
#if(GAMESERVER_UPDATE>=1200)//2017/06/02
		(lpItem->m_Index >= GET_ITEM(14,210) &&lpItem->m_Index <= GET_ITEM(14,213))||
		(lpItem->m_Index >= GET_ITEM(12,32) &&lpItem->m_Index <= GET_ITEM(14,34))||
#endif

		lpItem->m_Index == GET_ITEM(13,210) || lpItem->m_Index == GET_ITEM(13,220) || lpItem->m_Index == GET_ITEM(13,221) || lpItem->m_Index == GET_ITEM(13,222) || lpItem->m_Index == GET_ITEM(14,215) || lpItem->m_Index == GET_ITEM(14,216))
	{
		return 1;
	}
	else
...
void CEventInventory::CGEventItemGetRecv(PMSG_EVENT_ITEM_GET_RECV* lpMsg,int aIndex) // OK
{
	...
	pMsg.TmpIndex[0] = lpMsg->index[0];
	pMsg.TmpIndex[1]= lpMsg->index[1];
#endif
...
void CEventInventory::CGEventItemDropRecv(PMSG_EVENT_ITEM_DROP_RECV* lpMsg,int aIndex) // OK
{
	...
	if(gItemMove.CheckItemMoveAllowDrop(lpItem->m_Index) == 0)
	{
	 ...
	 if (gSummonScroll.CheckSummonScroll(lpItem->m_Index) != 0)
	{
...
#if(GAMESERVER_UPDATE>=1200)//2018/01/11
	if (gMuRummy.IsMuRummyEventOn() != 0 &&lpMsg->Type == 7)
#else
...
void CEventInventory::GCEventInventoryEnableSend(int aIndex,int Type) // OK
{
	// 0 = Event Inventory
	// 1 = Battle Core
	// 2 = egg
