...
struct PMSG_MUUN_ITEM_CONVERT_RECV
{
	PSBMSG_HEAD header; // C3:4E:13
	BYTE slot;
};
...
struct PMSG_MUUN_ITEM_CONVERT_SEND
{
	PSBMSG_HEAD header; // C1:4E:09
	BYTE result;
	DWORD money;
...
struct MUUN_SYSTEM_CONVERT_INFO
{
	int Index;
	int Convert;
	int ItemIndex;
...
public:
	...
	bool  IsMuunExpireDate(CItem *item);
	BYTE CheckMuunInventoryEmpty(LPOBJ lpObj);
	...
	bool GetMuunSystemConvertInfo(int index,MUUN_SYSTEM_CONVERT_INFO* lpInfo);
	bool CharacterUseConverterDur(LPOBJ lpObj,int SourceSlot,int TargetSlot);
	void CGMuunConvertRecv(PMSG_MUUN_ITEM_CONVERT_RECV* lpMsg,int aIndex);
...
private:
	...
	std::map<int,MUUN_SYSTEM_CONVERT_INFO> m_MuunSystemConvertInfo;
};
