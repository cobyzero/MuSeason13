...
void CPersonalShop::CGPShopOpenRecv(PMSG_PSHOP_OPEN_RECV* lpMsg,int aIndex) // OK
{
	...
	{
		GCMessagePopupSend(lpObj, "Você não pode abrir sua loja pessoal sem antes autenticar o PIN. Utilize o comando /pin seguido de seu PIN para faze-lo agora mesmo.");
		return;
	}
...
void CPersonalShop::CGPShopBuyItemRecv(PMSG_PSHOP_BUY_ITEM_RECV* lpMsg,int aIndex) // OK
{
	...
	{
		gCashShop.GDCashShopRecievePointSend(aIndex, (DWORD)&CPersonalShop::OnPShopBuyItemCallbackRecv, (DWORD)&gObj[bIndex], lpMsg->slot);
		return;
	}
}

void CPersonalShop::OnPShopBuyItemCallbackRecv(LPOBJ lpObj, LPOBJ lpTarget, DWORD slot, DWORD WCoinC, DWORD WCoinP, DWORD GoblinPoint) // OK
{
	...
	DWORD PShopWCCValue = lpTarget->Inventory[slot].m_PShopValue;
	...
	gPersonalShop.GetRequireJewelCount(lpObj, &RequireJewelCount[0], RequireJewelTable[0], 0, lpTarget->Inventory[slot].m_PShopJoBValue);
	gPersonalShop.GetRequireJewelCount(lpObj, &RequireJewelCount[1], RequireJewelTable[1], 1, lpTarget->Inventory[slot].m_PShopJoSValue);
	gPersonalShop.GetRequireJewelCount(lpObj, &RequireJewelCount[2], RequireJewelTable[2], 2, lpTarget->Inventory[slot].m_PShopJoCValue);

	gPersonalShop.GetPaymentJewelCount(lpTarget, &PaymentJewelCount[0], PaymentJewelTable[0], 0, lpTarget->Inventory[slot].m_PShopJoBValue);
	gPersonalShop.GetPaymentJewelCount(lpTarget, &PaymentJewelCount[1], PaymentJewelTable[1], 1, lpTarget->Inventory[slot].m_PShopJoSValue);
	gPersonalShop.GetPaymentJewelCount(lpTarget, &PaymentJewelCount[2], PaymentJewelTable[2], 2, lpTarget->Inventory[slot].m_PShopJoCValue);

	if (RequireJewelCount[0] < lpTarget->Inventory[slot].m_PShopJoBValue)
	{
		gPersonalShop.GCPShopBuyItemSend(lpObj->Index, lpTarget->Index, 0, 11);
		return;
	}

	if (RequireJewelCount[1] < lpTarget->Inventory[slot].m_PShopJoSValue)
	{
		gPersonalShop.GCPShopBuyItemSend(lpObj->Index, lpTarget->Index, 0, 12);
		return;
	}

	if (RequireJewelCount[2] < lpTarget->Inventory[slot].m_PShopJoCValue)
	{
		gPersonalShop.GCPShopBuyItemSend(lpObj->Index, lpTarget->Index, 0, 13);
		return;
	}
	if (lpTarget->Inventory[slot].m_PShopJoBValue > 0 && RequireJewelTable[0][0] == 0 && RequireJewelTable[0][1] == 0 && RequireJewelTable[0][2] == 0 && RequireJewelTable[0][3] == 0)
	{
		gPersonalShop.GCPShopBuyItemSend(lpObj->Index, lpTarget->Index, 0, 14);
		return;
	}

	if (lpTarget->Inventory[slot].m_PShopJoSValue > 0 && RequireJewelTable[1][0] == 0 && RequireJewelTable[1][1] == 0 && RequireJewelTable[1][2] == 0 && RequireJewelTable[1][3] == 0)
	{
		gPersonalShop.GCPShopBuyItemSend(lpObj->Index, lpTarget->Index, 0, 15);
		return;
	}

	if (lpTarget->Inventory[slot].m_PShopJoCValue > 0 && RequireJewelTable[2][0] == 0 && RequireJewelTable[2][1] == 0 && RequireJewelTable[2][2] == 0 && RequireJewelTable[2][3] == 0)
	{
		gPersonalShop.GCPShopBuyItemSend(lpObj->Index, lpTarget->Index, 0, 16);
		return;
	}

	if (gItemManager.GetInventoryEmptySlotCount(lpTarget) < (PaymentJewelCount[0] + PaymentJewelCount[1] + PaymentJewelCount[2]))
	{
		gPersonalShop.GCPShopBuyItemSend(lpObj->Index, lpTarget->Index, 0, 17);
		return;
	}
	...
	gLog.Output(LOG_TRADE, "[SellPesonalShopItem][%s][%s] - (Account: %s, Name: %s, Value: %d, WCCValue: %d, Index: %04d, Level: %02d, Serial: %08X, Option1: %01d, Option2: %01d, Option3: %01d, NewOption: %03d, JewelOfHarmonyOption: %03d, ItemOptionEx: %03d, SocketOption: %03d, %03d, %03d, %03d, %03d)", lpTarget->Account, lpTarget->Name, lpObj->Account, 0, PShopWCCValue, lpTarget->Inventory[slot].m_Index, lpTarget->Inventory[slot].m_Level, lpTarget->Inventory[slot].m_Serial, lpTarget->Inventory[slot].m_Option1, lpTarget->Inventory[slot].m_Option2, lpTarget->Inventory[slot].m_Option3, lpTarget->Inventory[slot].m_NewOption, lpTarget->Inventory[slot].m_JewelOfHarmonyOption, lpTarget->Inventory[slot].m_ItemOptionEx, lpTarget->Inventory[slot].m_SocketOption[0], lpTarget->Inventory[slot].m_SocketOption[1], lpTarget->Inventory[slot].m_SocketOption[2], lpTarget->Inventory[slot].m_SocketOption[3], lpTarget->Inventory[slot].m_SocketOption[4]);

	gCashShop.GDCashShopSubPointSaveSend(lpObj->Index, 0, PShopWCCValue, 0, 0);
	...
	gCashShop.GDCashShopAddPointSaveSend(lpTarget->Index, 0, PShopWCCValue, 0, 0);
	...
	gPersonalShop.GDPShopItemValueDeleteSaveSend(lpTarget->Index, slot);
