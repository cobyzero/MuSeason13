...
void CMonsterSkillElementInfo::ForceSkillElementInfo(int aIndex,int bIndex) // OK
{
	...
	switch(this->m_ElementType)
	{
		...
		case MSE_TYPE_BERSERK:
			...
			break;
		case MSE_TYPE_ICE_BREATH:
			this->ApplyElementIceBreathDamageAttack(aIndex, bIndex);
			break;
		case MSE_TYPE_ICE_BOOM:
			this->ApplyElementIceBoomDamageAttack( aIndex, bIndex);
			break;
		case MSE_TYPE_DEVIL_ICE:
			this->ApplyElementDevilIceDamageAttack( aIndex, bIndex);
			break;
	}
}
...
void  CMonsterSkillElementInfo::ApplyElementIceBreathDamageAttack(int iIndex, int iTargetIndex)
{


	if (gObjIsConnected(iIndex) == 0)
	{
		return;
	}
	...
	g_NixiesLakeBoss.InitIceBreathAttack( &gObj[iIndex], &gObj[iTargetIndex]);


}
void  CMonsterSkillElementInfo::ApplyElementIceBoomDamageAttack( int iIndex, int iTargetIndex)
{

	LPOBJ lpObj; // ST68_4@11
	...
	lpObj = &gObj[iIndex];
	LogAdd(LOG_BLACK, gMessage.GetMessage(540));
	gObjAddMsgSendDelay(lpObj, 74, iTargetIndex, this->m_NullifiedSkill, 0);

	return;
}
void  CMonsterSkillElementInfo::ApplyElementDevilIceDamageAttack(int iIndex, int iTargetIndex)
{
	if (gObjIsConnected(iIndex) == 0)
	{
		return;
	...
	gObjAddMsgSendDelay(&gObj[iIndex], 75, iTargetIndex, this->m_NullifiedSkill, 0);
	LogAdd(LOG_BLACK, gMessage.GetMessage(541));
