...
void CWarehouse::LoadDynamicVault(char* path) // OK
{
	...
	try
	{
		while (true)
		{
			...
			info.VaultNumber = lpMemScript->GetAsNumber();

			this->m_DynamicVaultInfo.insert(std::pair<int, DYNAMIC_VAULT_INFO>(info.VaultNumber, info));
		}
	}
	catch (...)
	{
...
int CWarehouse::CheckNPCVaultNumber(LPOBJ lpObj)
{
	for (std::map<int, DYNAMIC_VAULT_INFO>::iterator it = this->m_DynamicVaultInfo.begin(); it != this->m_DynamicVaultInfo.end(); it++)
	{
		if (it->second.Map == lpObj->Map && it->second.X == lpObj->X && it->second.Y == lpObj->Y)
		{
			return it->second.VaultNumber;
		}
	}
