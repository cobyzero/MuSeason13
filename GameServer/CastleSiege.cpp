...
BOOL CCastleSiege::SetCastleInitData(CSP_ANS_CSINITDATA* lpMsg)
{
	...
	LogAdd(LOG_BLACK,"[CastleSiege] CCastleSiege::LoadData() - Siege Schedule Start Date (%d-%d-%d)", m_tmStartDate.wYear, m_tmStartDate.wMonth, m_tmStartDate.wDay);
	
	LogAdd(LOG_BLACK,"[CastleSiege] CCastleSiege::LoadData() - Siege Schedule Date (%d-%d-%d)", EndTime.wDay, EndTime.wHour, EndTime.wMinute);

	GetNextDay(&m_tmSiegeEndSchedule, EndTime.wDay, EndTime.wHour, EndTime.wMinute, 0);

	LogAdd(LOG_BLACK,"[CastleSiege] CCastleSiege::LoadData() - Siege Schedule End Date (%d-%d-%d(%d:%d:%d))", this->m_tmSiegeEndSchedule.wYear, this->m_tmSiegeEndSchedule.wMonth, this->m_tmSiegeEndSchedule.wDay, this->m_tmSiegeEndSchedule.wHour, this->m_tmSiegeEndSchedule.wMinute, this->m_tmSiegeEndSchedule.wSecond);
	...
	if( (gObj[this->m_iCastleCrownAccessUser].CsJoinSide >= 2 ) && (gObj[this->m_iCastleSwitchAccessUser1].CsJoinSide >= 2 ) && (gObj[this->m_iCastleSwitchAccessUser2].CsJoinSide >= 2 ) )
	{
		if( gObj[this->m_iCastleCrownAccessUser].CsGuildInvolved == 1 )
		{
			if( (gObj[this->m_iCastleCrownAccessUser].CsJoinSide == gObj[this->m_iCastleSwitchAccessUser1].CsJoinSide) && (gObj[this->m_iCastleCrownAccessUser].CsJoinSide == gObj[this->m_iCastleSwitchAccessUser2].CsJoinSide) )
			{
				...
				{
					gCastleSiege.SaveSiegeCharInfo(&gObj[this->m_iCastleCrownAccessUser], 5);
					
					LogAdd(LOG_BLACK,"[CastleSiege] Castle Winner Has been Changed (GUILD:%s)", gObj[this->m_iCastleCrownAccessUser].GuildName);

					gCastleSiege.SaveSiegeCharInfo(&gObj[this->m_iCastleCrownAccessUser], 13); //reset accu time
					...
					//Send popup notifying the register
					for (int i = OBJECT_START_USER; i < MAX_OBJECT; i++)
					{
						if (gObj[i].Connected == OBJECT_ONLINE && gObj[i].Type == OBJECT_USER && gObj[i].Map == MAP_CASTLE_SIEGE)
						{
							GCMessagePopupSend(&gObj[i], gMessage.GetMessage(40), gObj[this->m_iCastleCrownAccessUser].GuildName);
						}
					}

					GCFireworksSend(&gObj[this->m_iCastleCrownAccessUser], (gObj[this->m_iCastleCrownAccessUser].X + (((GetLargeRand() % 5) * 2) - 4)), (gObj[this->m_iCastleCrownAccessUser].Y + (((GetLargeRand() % 5) * 2) - 4)));
					GCFireworksSend(&gObj[this->m_iCastleCrownAccessUser], (gObj[this->m_iCastleCrownAccessUser].X + (((GetLargeRand() % 5) * 2) - 4)), (gObj[this->m_iCastleCrownAccessUser].Y + (((GetLargeRand() % 5) * 2) - 4)));
					GCFireworksSend(&gObj[this->m_iCastleCrownAccessUser], (gObj[this->m_iCastleCrownAccessUser].X + (((GetLargeRand() % 5) * 2) - 4)), (gObj[this->m_iCastleCrownAccessUser].Y + (((GetLargeRand() % 5) * 2) - 4)));
					GCFireworksSend(&gObj[this->m_iCastleCrownAccessUser], (gObj[this->m_iCastleCrownAccessUser].X + (((GetLargeRand() % 5) * 2) - 4)), (gObj[this->m_iCastleCrownAccessUser].Y + (((GetLargeRand() % 5) * 2) - 4)));
					GCFireworksSend(&gObj[this->m_iCastleCrownAccessUser], (gObj[this->m_iCastleCrownAccessUser].X + (((GetLargeRand() % 5) * 2) - 4)), (gObj[this->m_iCastleCrownAccessUser].Y + (((GetLargeRand() % 5) * 2) - 4)));
...
void CCastleSiege::NotifyCrownSwitchInfo(int iCrownSwitchIndex)
{
	...
	PMSG_ANS_NOTIFY_CROWNSWITCH_INFO pMsg2;
	pMsg2.h.set(0xB2, 0x20, sizeof(pMsg2));

	pMsg.switch_index = pMsg2.switch_index = iCrownSwitchIndex;
	pMsg.id = pMsg2.id = gObj[iCrownSwitchIndex].Class - 217;

	pMsg.state = pMsg2.state = 0;
	pMsg.join_side = pMsg2.join_side = 0;

	memset(&pMsg.guild, 0, 8);
	memset(&pMsg.user, 0, 10);
	memset(&pMsg2.guild, 0, 8);
	memset(&pMsg2.user, 0, 10);

	memset(&pMsg.crownGuild, 0, 8);
	memset(&pMsg.crownUser, 0, 10);
	memset(&pMsg2.crownGuild, 0, 8);
	memset(&pMsg2.crownUser, 0, 10);
	pMsg.crownState = pMsg2.crownState = 0;
	pMsg.accumulated_time = pMsg2.accumulated_time = 0;

	int crownUserIndex = gCastleSiege.GetCrownUserIndex();

	if (gObjIsConnected(crownUserIndex))
	{
		LPOBJ lpCrownObj = &gObj[crownUserIndex];
		memcpy(&pMsg.crownUser, lpCrownObj->Name, 10);
		memcpy(&pMsg.crownGuild, lpCrownObj->GuildName, 8);
		pMsg.crownState = TRUE;
		pMsg.accumulated_time = lpCrownObj->AccumulatedCrownAccessTime + (GetTickCount() - this->m_dwCrownAccessTime);
	}
	...
	if( gObjIsConnected(iCrownSwitchUserIndex) )
	{
		...
		pMsg.state = TRUE;
		pMsg.join_side = lpSwitchObj->CsJoinSide;

		memcpy(&pMsg.user, lpSwitchObj->Name, 10);
		memcpy(&pMsg.guild, lpSwitchObj->GuildName, 8);
		memset(&szMasterGuildName, 0, 8);

		if( lpSwitchObj->Guild )
		{
			if (lpSwitchObj->Guild->GuildUnion != 0)
			{
				if (gUnionManager.GetUnionName(lpSwitchObj->Guild->GuildUnion, szMasterGuildName) != -1)
				{
					memcpy(&pMsg.guild, szMasterGuildName, 8);
				}
			}
		}
	}
	...
	for (int i = OBJECT_START_USER; i < MAX_OBJECT; i++)
	{
		if (gObj[i].Connected == OBJECT_ONLINE && gObj[i].Type == OBJECT_USER && gObj[i].Guild)
		{
			gObj[i].Guild->membersInTower = 0;
		}
	}
	...
	for (int i = OBJECT_START_USER; i < MAX_OBJECT; i++)
	{
		if (gObj[i].Connected == OBJECT_ONLINE && gObj[i].Type == OBJECT_USER)
		{
			if (this->IsInDragonTower(&gObj[i]))
			{
				LPOBJ lpObj = &gObj[i];
				...
				if (lpObj->Guild)
				{
					if (lpObj->Guild->GuildUnion != 0)
					{
						...
						if (gUnionManager.GetUnionName(lpObj->Guild->GuildUnion, szMasterGuildName) != -1)
						{
							...
							mainGuild->membersInTower++;
						}
					}
					else
					{
						lpObj->Guild->membersInTower++;
					}
				}
			}
...
bool CCastleSiege::IsInDragonTower(LPOBJ lpObj)
{
	if (lpObj->Map != MAP_CASTLE_SIEGE)
		return false;

	return ((lpObj->X >= 159 && lpObj->X <= 190) && (lpObj->Y >= 189 && lpObj->Y <= 217));
}

void CCastleSiege::SaveSiegeCharInfo(LPOBJ lpObj, int Type, int Value)
{
	if (!OBJECT_RANGE(lpObj->Index))
		return;
	...
	gDataServerConnection.DataSend((BYTE*)&pMsg, sizeof(SAVE_CHAR_SIEGE_INFO));
}

void CCastleSiege::GetSiegeCharInfo(LPOBJ lpObj)
{
	if (!OBJECT_RANGE(lpObj->Index))
		return;
