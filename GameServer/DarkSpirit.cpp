...
bool CDarkSpirit::Attack(LPOBJ lpObj,LPOBJ lpTarget,CSkill* lpSkill,bool send,BYTE flag,BYTE action) // OK
{
...
#if(GAMESERVER_UPDATE>=902)//2017/05/11
	gAttack.HelperSprite(lpObj,lpTarget,&damage);
#else
	gAttack.HelperSprite(0,lpTarget,&damage);
#endif

	int MinDamage = (lpObj->Level + lpObj->MasterLevel + lpObj->MasterLevel) / 10;
	...
	gAttack.MountSprite(lpTarget, damage);
	...
	if( lpTarget->Inventory[lpTarget->m_btInvenPetPos].IsItem() == 0 ||(lpTarget->Inventory[lpTarget->m_btInvenPetPos].m_Index != GET_ITEM(13,2) && lpTarget->Inventory[lpTarget->m_btInvenPetPos].m_Index != GET_ITEM(13,3) && lpTarget->Inventory[lpTarget->m_btInvenPetPos].m_Index != GET_ITEM(13,4) &&lpTarget->Inventory[lpTarget->m_btInvenPetPos].m_Index!= GET_ITEM(13,37)))
#else
...
void CDarkSpirit::DarkSpiritSprite(LPOBJ lpObj,int damage) // OK
{
	...
	MaxDurSmall = (MaxDurSmall*lpObj->m_MPSkillOpt.PetDurabilityRate)/100;
...
{
	if(this->m_AttackSuccessRate < lpTarget->m_MPSkillOpt.DefenseSuccessRate)
	{
		(*miss) = 1;
	...
	else
	{
		...
		if((GetLargeRand()%((this->m_AttackSuccessRate==0)?1:this->m_AttackSuccessRate)) < lpTarget->m_MPSkillOpt.DefenseSuccessRate)
		{
			gAttack.MissSend(lpObj,lpTarget,lpSkill,send,0);
			return 0;
		}
