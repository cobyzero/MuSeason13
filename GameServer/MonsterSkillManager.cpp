...
CMonsterSkillUnitInfo* CMonsterSkillManager::FindMonsterSkillUnitInfo(int aIndex,int MonsterSkillUnitType) // OK
{
	...
	if (MonsterSkillUnitTypeCount!=0)
	{
		//if (lpObj->Class == 716 && !MonsterSkillUnitTypeCount)
		//	MonsterSkillUnitTypeCount = CCoreMagriffi::GetAvailableAttackSkillCount(&g_CoreMagriffi, lpObj);
		//if (lpObj->Class == 734)
		//	iFoundSkillCount = FereaBoss::GetAttackSkillCount(&g_FereaBoss, lpObj);
		if (lpObj->Class == 746 && !MonsterSkillUnitTypeCount)
			MonsterSkillUnitTypeCount = g_NixiesLakeBoss.GetAvailableAttackSkillCount(lpObj);
	}
#endif
