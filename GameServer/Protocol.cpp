...
{
	//LogAdd(LOG_DEBUG, "[RECVClient]: %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x", lpMsg[0], lpMsg[1], lpMsg[2], lpMsg[3], lpMsg[4], lpMsg[5], lpMsg[6], lpMsg[7], lpMsg[8], lpMsg[9], lpMsg[10], lpMsg[11], lpMsg[12], lpMsg[13], lpMsg[14], lpMsg[15], lpMsg[16], lpMsg[17], lpMsg[18], lpMsg[19]);
	...
	{
		LogAdd(LOG_RED, "[RECV]: %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x %x"
	...
	case 0x00:
		...
		break;
	case 0x01:
		CGCharRecv((PMSG_CHATDATA_NUMBER*)lpMsg, aIndex);
	case 0x02:
	...
	case 0x18:
		...
		break;
	case 0x19:
		if (gServerInfo.FilaHit == 1)
			g_FilaHit.Add(1, aIndex, lpMsg);
		else
	...
	case 0x34:
		...
		break;
	case 0x35:
		gItemManager.GCItemDisintegration((PMSG_ITEM_DISINTEGRATION_RECV*)lpMsg, aIndex);
		break;
	case 0x36:
	...
	case 0x3E:
		...
		{
		case 0x20:
			gFourthQuest.CGCentWarpToTestArea(aIndex);
			break;
		case 0x21:
			gFourthQuest.CGCentClosePopupAndStartQuest(aIndex);
			break;
		case 0x22:
			gFourthQuest.CGCentWarpToAreaOfAlegibility(aIndex);
			break;
		}
		break;
	case 0x3F:
	...
	case 0x4D:
		...
		case 0x15:
			...
			//case 0x19:
			//	CMuRummyMng::CGReqCardMatch4Slot(&g_CMuRummyMng, (_tagPMSG_REQ_CARD_MATCH_4SLOT *)lpClientTime, index);
			//	break;
		case 0x23:
			g_CBombHuntMng.CGReqBombHuntStart(aIndex);
			break;
		case 0x24:
			g_CBombHuntMng.CGReqLBClick((_tagPMSG_REQ_BOMB_HUNT_LB_CLICK*)lpMsg, aIndex);
			break;
		case 0x25:
			g_CBombHuntMng.CGReqRBClick((_tagPMSG_REQ_BOMB_HUNT_RB_CLICK*)lpMsg, aIndex);
			break;
		case 0x27:
			g_CBombHuntMng.CGReqBombHuntBanner(aIndex);
			break;
		case 0x28:
			g_CBombHuntMng.CGReqReward(aIndex);
			break;
		}
		break;
	case 0x4E:
		...
		case 0x09:
			...
			break;
		case 0x11:
			gMuunSystem.CGReqRideSelect((PMSG_MUUN_RIDE_SELECT*)lpMsg, aIndex);
			break;
		case 0x13:
			gMuunSystem.CGMuunConvertRecv((PMSG_MUUN_ITEM_CONVERT_RECV*)lpMsg, aIndex);
			break;
		}
		break;
	case 0x4F:
	{
		...
		{
		case 0x02:
			g_GremoryCase.CGReqGetItemFromGremoryCase((PMSG_ADD_GREMORYCASE_ITEM_TO_INVENTORY_REQ*)lpMsg, aIndex);
			break;
		case 0x05:
	...
	case 0x6E:
		...
		{
		case 0x01:
			//g_CBlockChatUserSystem.CG_ReqAddBlockChattingUser(
			//	(PMSG_REQ_ADD_BLOCK_CHAT_USER *)lpMsg,
			//	aIndex);
			break;
		case 0x02:
			//g_CBlockChatUserSystem.CG_ReqDelBlockChattingUser(
			//	(PMSG_REQ_DEL_BLOCK_CHAT_USER *)lpMsg,
			//	aIndex);
			break;
		}
		break;
#if(GAMESERVER_UPDATE>=1100)//2018/07/21 ��Ʒ�ع�
	case 0x6F:
		if (lpMsg[3])
		{
			if (lpMsg[3] == 1)
			{
				gCRestoreSoldItem.CGReqQuitRestoreItemInterface(
					(PMSG_REQ_QUIT_RESTORE_INTERFACE*)lpMsg,
					aIndex);
			}
			else if (lpMsg[3] == 2)
			{
				if (!gObj[aIndex].restoringFlag)
				{
					gObj[aIndex].restoringFlag = 1;
					gCRestoreSoldItem.CGReqRestoreItem((PMSG_REQ_RESTORE_ITEM*)lpMsg, aIndex);
					gObj[aIndex].restoringFlag = 0;
				}
		...
		{
			gCRestoreSoldItem.CGReqRestoreItemList(
				(PMSG_REQ_RESTOREITEM_LIST*)lpMsg,
				aIndex,
				0);
		}
		break;
		...
		//case 0x70:
		//	v64 = lpClientTime;
		//	v13 = *((_BYTE *)&lpClientTime->h + 3);
		//	if (v13)
		//	{
		//		if (v13 == 1)
		//			LuckyItemManager::CGReqLuckyItemSmelting(
		//				&g_LuckyItemManager,
		//				(PMSG_REQ_LUCKYITEM_SMELTING *)lpClientTime,
		//				index);
		//	}
		//	else
		//	{
		...
		//			&g_LuckyItemManager,
		//			(PMSG_REQ_TICKET_EXCHANGE *)lpClientTime,
		//			index);
		//	}
		//	break;
	case 0x71:
		gCheatGuard.GCPingSendRecv((PMSG_PING *)lpMsg, aIndex);
		break;
		//case 0x72:
		//	if (DataEncryptCheck(index, a1, Encrypta))
		//		GCPacketCheckSumRecv((PMSG_PACKETCHECKSUM *)lpClientTime, index);
		//	break;
		//case 0x73:
		//	if (DataEncryptCheck(index, a1, Encrypta))
		//		GCNPggCheckSumRecv((PMSG_NPROTECTGGCHECKSUM *)lpClientTime, index);
		//	break;
	...
	case 0x76:
		...
		{
		case 0x01:
			g_NixiesLakeBossZone.RecvReqChkEnterBossZone((_tagPMSG_REQ_CHK_ENTER_ON_NIXIES_NIX_BOSS_ZONE*)lpMsg, aIndex);
			break;
		case 0x02://�볡
			g_NixiesLakeBossZone.RecvReqEnterBossZone((_tagPMSG_REQ_ENTER_ON_NIXIES_NIX_BOSS_ZONE*)lpMsg, aIndex);
			break;
		case 0x03:
			g_NixiesLakeBossZone.RecvReqEnterBossZoneByInvite((_tagPMSG_ANS_INVITE_ON_NIXIES_NIX_BOSS_ZONE_WITH_PARTYMEMBER*)lpMsg, aIndex);
			break;
		case 0x10:
			g_NixiesLakeBoss.RecvSendIceBreathAttack((_tagPMSG_ICE_BREATH_ATTACK*)lpMsg, aIndex);
			break;
		}
		break;
	...
	/*case 0x77:
		...
		{
		case 0x00:
			g_Labyrinth.CGReqCheckLabyrinthEntering(aIndex);
			break;
		case 0x01:
			g_Labyrinth.CGReqEnterLabyrinth(aIndex);
			break;
		case 0x05:
			g_Labyrinth.CGReqMoveLabyrinthZone(aIndex, (PMSG_REQ_MOVE_LABYRINTH_ZONE*)lpMsg);
			break;
		case 0x11:
			g_Labyrinth.CGReqLevelUpLabyrinth(aIndex, (PMSG_REQ_LABYRINTH_DIMENSION_LEVEL_UP*)lpMsg);
			break;
		}
		break;*/
	case 0x78:
		gSocketChaos.CGSocketChaosMix((PMSG_SOCKET_MIX_RECV*)lpMsg, aIndex);
		//case 1:
		//	CGReqUpgradeSocketItem((PMSG_REQ_UPGRADE_SOCKETITEM *)lpClientTime, index);
		//	break;
		//case 2:
		//	CGReqExtractSeedInven((PMSG_REQ_EXTRACT_SEED_INVEN *)lpClientTime, index);
		//	break;
		//case 3:
		//	CGReqSetSeedSphearInven((PMSG_REQ_SET_SEEDSPHERE_INVEN *)lpClientTime, index);
		//	break;
		//case 4:
		//	CGReqRemoveSeedSphearInven((PMSG_REQ_REMOVE_SEEDSPHERE_INVEN *)lpClientTime, index);
		//	break;
		//case 5:
		//	CGReqCompositeSeedSphearInven((PMSG_REQ_COMPOSITE_SEEDSPHERE_INVEN *)lpClientTime, index);
		//	break;
		//case 6:
		//	CGReqUpgradeSeedSphearInven((PMSG_REQ_UPGRADE_SEEDSPHERE_INVEN *)lpClientTime, index);
		//	break;
		//case 7:
		//	CGReqDeleteSocket((PMSG_REQ_DELETE_SOCKET *)lpClientTime, index);
		//	break;
		break;
	case 0x79:
		gCheatGuard.HardIdRecv(aIndex, (HARDWARE_ID_RECV*)lpMsg);
		break;
	case 0x7e:
		switch (lpMsg[3])
		{
		case 0x01:
			gMasterSkillTree_4th.CGMasterSkillRecv((PMSG_FOR_MASTER_SKILL_RECV*)lpMsg, aIndex);
			break;
		}
		break;
	case 0x81:
	...
	case 0x87:
		...
		break;
	case 0x88:
		//CGLQChaosBoxSpaceCheck((PMSG_REQ_LQMIX_SPACE_CHECK *)lpClientTime, index);
		//gChaosBox.CGChaosMixRecvMP((PMSG_CHAOS_MIX_RECV*)lpMsg,aIndex);

		PMSG_CHAOS_MIX_SEND_MP pMsg;
	...
	case 0x91:
		...
		break;
		//case 0x95:
		//	GCRegEventChipRecv((PMSG_REGEVENTCHIP *)lpClientTime, index);
		//	break;
		//case 0x96:
		//	GCGetMutoNumRecv((PMSG_GETMUTONUMBER *)lpClientTime, index);
		//	break;
		//case 0x97:
		//	GCUseEndEventChipRescv(index);
		//	break;
		//case 0x98:
		//	GCUseRenaChangeZenRecv((PMSG_EXCHANGE_EVENTCHIP *)lpClientTime, index);
		//	break;
		//case 0x99:
		//	CGReqMoveOtherServer((PMSG_REQ_MOVE_OTHERSERVER *)lpClientTime, index);
	case 0x9A:
		gBloodCastle.CGBloodCastleEnterRecv((PMSG_BLOOD_CASTLE_ENTER_RECV*)lpMsg, aIndex);
		break;
		//case 0x9F:
		//	CGRequestEventEnterCount((PMSG_REQ_CL_ENTERCOUNT *)lpClientTime, index);
		//	break;
	case 0xA0:
	...
	case 0xAF:
		...
		case 0x02:
			...
			break;
			//case 3:
			//	CGReqCCF_DayTime(index);
			//	break;
			//case 5:
			//	CGReqCCF_EnterCheck(index);
			//	break;
			//case 6:
			//	CGRequestRepositionUserInCCF((PMSG_REQ_REPOSUSER_IN_CCF *)lpClientTime, index);
			//	break;
			//case 7:
			//	CGReq_CCF_Ranking((PMSG_REQ_CCF_RANKING *)lpClientTime, index);
			//	break;
			//case 8:
			//	CG_CCF_UI_OnOff((PMSG_CCF_UI_ONOFF *)lpClientTime, index);
		}
		break;
	case 0xB0:
	...
	case 0xBF:
		...
		case 0x51:
			...
			break;
		case 0x52: //set assign helper (1)
			break;
		case 0x56: //set auto movement (1)
			break;

			//case 0x5A:
			//	CGEnterCheckITL((PMSG_REQ_ENTER_ITL *)lpClientTime, index);
			//	break;
			//case 0x5B:
			//	CGReqAcceptEnterITL((PMSG_REQ_ACCEPTENTER *)lpClientTime, index);
			//	break;
			//case 0x5C:
			//	CGReqITLSchedule(index);
			//	break;
			//case 0x5D:
			//	CGReqITLGuild_LeagueRank(index);
			//	break;
			//case 0x5E:
			//	CGReqITLTournamentRank(index);
			//	break;
			//case 0x5F:
			//	CGReq_ITL_ReEnter(index);
			//	break;
			//case 0x67:
			//	CGReqITLMagicUse((PMSG_ITL_USE_MAGIC *)lpClientTime, index);
			//	break;
			//case 0x68:
			//	CGReq_RewardList(index);
			//	break;
			//case 0x69:
			//	CGReq_ITLGetReward(index);
			//	break;
			//case 0x6A:
			//	CGReq_ITL_Relics((PMSG_REQ_ITL_RELATE_RELICS *)lpClientTime, index);
			//	break;
			//case 0x6B:
			//	CGReq_ITL_Reward_Gem((PMSG_ITL_REQ_REWARD *)lpClientTime, index);
			//	break;

		case 0x70:
			gIllusionTemple.CGIllusionTempleEnterRecv((PMSG_ILLUSION_TEMPLE_ENTER_RECV*)lpMsg, aIndex);
			//case 0x71:
			//	CG_ReqAcceptEnterITR((PMSG_REQ_ACCEPTENTER_ITR *)lpClientTime, index);
			//	break;
			break;
		}
		break;
	...
	case 0xCD:
		...
		{
		case 0x06:
			GremoryCase((PMSG_GREMORY_CASE*)lpMsg, aIndex);
			break;
		}
		//switch (*((_BYTE *)&lpClientTime->h + 3))
		//{
		//case 1:
		//	CGReqUBFMyCharacterInfo((PMSG_REQ_UBF_INFO *)lpClientTime, index);
		//	break;
		//case 2:
		//	CGReqUBFJoin((PMSG_REQ_UBF_JOIN *)lpClientTime, index);
		//	break;
		//case 6:
		//	CGReqUBFGetReward((PMSG_REQ_GET_REWARD *)lpClientTime, index);
		//	break;
		//case 7:
		//	CGReqUBFCancel((PMSG_REQ_UBF_CANCEL *)lpClientTime, index);
		//	break;
		//case 8:
		//	CGReqUBFGetRealName((PMSG_REQ_UBF_REAL_NAME *)lpClientTime, index);
		//	break;
		break;
	...
	case 0xD0:
		...
		case 0x10:
			...
			break;
		case 0xF0:
			gRheaGold.RheaItemRecv((PMSG_RHEA_ITEM_BUY_RECV*)lpMsg, aIndex);
			break;
		case 0xF2:
			gItemManager.GCItemMouseRecv((PMSG_ITEM_MOUSE_RECV*)lpMsg, aIndex);
			break;
		}
		break;
	case 0xD1:
	...
	case 0xD2:
		...
		case 0x05:
			...
			break;
			//case 7:
			//	CCashShopInGame::RequestGiftCash(&g_CashShopInGame, lpObj, (PMSG_CASHSHOP_CASHSEND_REQ *)lpClientTime);
			//	break;
			//case 8:
			...
			//		&g_CashShopInGame,
			//		lpObj,
			//		(PMSG_CASHSHOP_ITEMBUY_LEFT_COUNT_REQ *)lpClientTime);
			//	break;
			//case 0xA:
			//	CCashShopInGame::RequestStorageItemThrow(
			//		&g_CashShopInGame,
			//		lpObj,
			//		(PMSG_CASHSHOP_STORAGE_ITEM_THROW_REQ *)lpClientTime);
			//	break;
		case 0x0B:
			gCashShop.CGCashShopItemUseRecv((PMSG_CASH_SHOP_ITEM_USE_RECV*)lpMsg, aIndex);
			break;
		case 0x0F:
			gCashShop.CGCashShopItemUseRecvSXI((PMSG_CASH_SHOP_ITEM_USE_RECV_SXI*)lpMsg, aIndex);
		default:
	...
	case PROTOCOL_CODE4:
		...
		//	{
		//	case 0:
		//		CGReqDSFSchedule(index);
		//		break;
		//	case 1:
		//		GDReqCanPartyEnter(index);
		//		break;
		//	case 2:
		//		CGReqAcceptEnterDSF((PMSG_REQ_DSF_ACCEPTENTER *)lpClientTime, index);
		//		break;
		//	case 3:
		//		CGReqDSFGoFinalParty((PMSG_REQ_DSF_GO_FINAL_PARTY_INFO *)lpClientTime, index);
		//		break;
		//	case 9:
		//		CGReqDSFGetReward((PMSG_REQ_DSF_GET_REWARD *)lpClientTime, index);
		//		break;
		//	default:
		//		return;
		//	}
		//	break;
		//case 0xDD:
		//	v19 = *((_BYTE *)&lpClientTime->h + 3);
		//	if (v19 == 1)
		//	{
		//		CGReqDSNEnter((PMSG_REQ_DSN_ENTER *)lpClientTime, index);
		//	}
		//	else if (v19 == 2)
		//	{
		//		CGReqAcceptEnterDSN((PMSG_REQ_DSN_ACCEPTENTER *)lpClientTime, index);
		//	}
		//	break;
	case 0xE1:
	...
	case 0xE7:
		...
		break;
	case 0xE8:
		gItemManager.CGOverlapItemSplitRequest((PMSG_REQ_OVERLAP_ITEM_SPLIT *)lpMsg, aIndex);
		break;
	case 0xE9:
	...
	case 0xEC:
		...
		case 0x03:
			...
			break;
		case 0x05:
#if(GAMESERVER_UPDATE>=1100)//2017/04/25 ��ϲ���
			//CGReqPentagramSlotOpen((PMSG_REQ_PENTAGRAM_SLOT_OPEN *)lpClientTime, index);
			gPentagramSystem.CGPentagramEcstasySlotRecv((PMSG_PENTAGRAM_ECSTASY_SLOT_RECV*)lpMsg, aIndex);
#endif
			break;
		case 0x31:
#if(GAMESERVER_UPDATE>=802)
		...
		case 0x33:
			...
			break;
		case 0x50:
			gCGHuntingGrounds.CGReqHuntingRecordWndtOpen((PMSG_REQ_HUNTING_RECORD_WND_OPEN*)lpMsg, aIndex);
			break;
		case 0x51:
			gCGHuntingGrounds.CGReqHuntingRecordWndtClose((PMSG_REQ_HUNTING_RECORD_WND_CLOSE*)lpMsg, aIndex);
			break;
		case 0x52:
			//����
			gCGHuntingGrounds.CGReqHuntingRecordInfoUserOpen((PMSG_REQ_HUNTING_RECORD_INFO_USER_OPEN*)lpMsg, aIndex);
			break;
		case 0x54:
			//��ѯ������ɫ����
			gCGHuntingGrounds.CGReqHuntingRecordInfoUserOpenGet((PMSG_REQ_HUNTING_RECORD_INFO_USER_OPEN_GET*)lpMsg, aIndex);
			break;
#if(GAMESERVER_UPDATE>=1000)//2017/02/28
		case 0x27:
			//CGReq_DetailInfo((PMSG_REQ_DETAILINFO_FIRST *)lpClientTime, index);
			SXGetCharacterInfo(aIndex);
			SXGetMosaic(aIndex);
			break;
#endif
		}
	...
	case 0xEF:
		...
		case 0x06:
			...
			break;
			//case 7:
			//	CGReqPartyLeaderChange((_PMSG_REQ_CHANGE_PARTY_LEADER *)lpClientTime, index);
			//	break;
		}
		break;
	case 0xF1:
		...
		case 0x02:
			...
			break;
		case 0x03:
			//CGClientMsg((PMSG_CLIENTMSG *)lpClientTime, index);
			break;
		case 0x0C:
			break;
		}
	...
	case 0xF3:
		...
		case 0x21:
			...
			break;
		case 0x26:
			CGReqDoUsePopupType((PMSG_REQ_USE_POPUP*)lpMsg, aIndex);
			break;
		case 0x30:
	...
	case 0xF6:
		...
		case 0x0F:
			...
			break;
			//case 0x1A:
			//	CGReqProgressQuestList((PMSG_REQ_QUESTEXP_PROGRESS_LIST *)lpClientTime, index);
			//	break;
			//case 0x1B:
			//	CGReqProgressQuestInfo((PMSG_REQ_QUESTEXP_PROGRESS_INFO *)lpClientTime, index);
			//	break;
		case 0x10:
		...
		case 0x31:
			...
			break;
			//case 0x21:
			//	CGReqEventItemQuestList((PMSG_REQ_EVENT_ITEM_EP_LIST *)lpClientTime, index);
			//	break;
		case 0x51:
			gNewQuestWorld.CGNewQuestWorldAward((PMSG_NEW_QUEST_WORLD_RECV*)lpMsg, aIndex);
			break;
		case 0x52:
			gNewQuestWorld.CGNewQuestWorldGiveUp((PMSG_NEW_QUEST_WORLD_RECV*)lpMsg, aIndex);
			break;
		case 0x53:
			gNewQuestWorld.CGNewQuestWorldGiveUpChapter((PMSG_NEW_QUEST_WORLD_RECV*)lpMsg, aIndex);
			break;
		case 0x54:
			gNewQuestWorld.CGNewQuestWorldGoNow((PMSG_NEW_QUEST_WORLD_RECV*)lpMsg, aIndex);
			break;
		case 0x55:
			gNewQuestWorld.CGNewQuestWorldButtonMove((PMSG_NEW_QUEST_WORLD_RECV*)lpMsg, aIndex);
			break;
		}
		break;
	case 0xF7:
	...
	case 0xFC:
		...
		//	break;
		//case 0xFD:
		//	CGReqUserSettingSave((PMSG_USERSETTING_SAVEDATA *)lpClientTime, index);
		//	break;
...
void CGChatRecv(PMSG_CHAT_RECV* lpMsg, int aIndex) // OK
{
	...
	int nameLength = strlen(lpMsg->name) >= 10 ? 10 : strlen(lpMsg->name);
	strncpy(name, lpMsg->name, nameLength);
...
#if(IAJulia==1)
	if (Chat.ChatDataSend(lpObj, lpMsg)) return; 
#else
	if (lpMsg->message[0] == '/')
	{
		gCommandManager.ManagementCore(&gObj[aIndex], lpMsg->message);
		return;		
	} 
#endif
	...
	char tempMessage[61] = { 0 };	
	strncpy(tempMessage, lpMsg->message, strlen(lpMsg->message));
	memcpy(pMsg.message, tempMessage, sizeof(pMsg.message));
	...
	else if (lpMsg->message[0] == '@')
	{
		if (lpObj->Guild != 0)
		{
			...
			else if (lpMsg->message[1] == '@')
			{
				...
				{
					GDUnionServerGroupChattingSend(lpObj->Guild->GuildUnion, name, lpMsg->message);
				}
			}
			else
			{
				...
				{
					GDGuildServerGroupChattingSend(lpObj->Guild->Number, name, lpMsg->message);
				}
			}
		}
	}
	...
	else if (lpMsg->message[0] == '!')
	{
		if (lpMsg->message[1] == '!')
		{
			if (lpMsg->message[2] == '!')
			{
				if (lpMsg->message[3] == '!')
				{
					gCommandManager.CommandGMPost(lpObj, 3, &lpMsg->message[4]);
				}
				else
				{
					gCommandManager.CommandGMPost(lpObj, 2, &lpMsg->message[3]);
				}
			}
			else
			{
				gCommandManager.CommandGMPost(lpObj, 1, &lpMsg->message[2]);
			}
		}
		else
		{
			gCommandManager.CommandGMPost(lpObj, 0, &lpMsg->message[1]);
		}
	}
	else
	{
		...
		if (lpObj->Map != MAP_Labyrinth
			...
			&& !gCastleSiege.IsInDragonTower(lpObj)
#endif
			) 
		{
...
#if(IAJulia==1)
	if (Chat.WisperChat(lpObj, lpMsg)) return; 
#endif
	if (lpTarget == 0)
	{
...
void CGMainCheckRecv(PMSG_MAIN_CHECK_RECV* lpMsg, int aIndex) // OK
{
	...
	gCheatGuard.GCSendPing(lpObj->Index);

#if(IAJulia==1)
	Addons.LoginMsg(lpObj);
#endif
}
...
void CGMoveRecv(PMSG_MOVE_RECV* lpMsg, int aIndex) // OK
{
	...
	//#if(GAMESERVER_UPDATE>=1200)//2018/01/04
...
void CGConnectAccountRecv(PMSG_CONNECT_ACCOUNT_RECV* lpMsg, int aIndex) // OK
{
	...
	if (lpObj->LoginMessageSend == 0)
	{
		...
		//Force lower case to re-re-re-avoid dupe account
		for (int i = 0; i < 10; i++)
		{
		...
		//LogAdd(LOG_BLUE, "Auth user [%s][%s]", account, password);
...
void CGCharacterCreateRecv(PMSG_CHARACTER_CREATE_RECV* lpMsg, int aIndex) // OK
{
	...
	if (
#if (GAMESERVER_UPDATE>=1000)//2017/02/28
		lpMsg->Class != DB_CLASS_FK &&
		lpMsg->Class != DB_CLASS_PK &&
#endif
		lpMsg->Class != DB_CLASS_DW && lpMsg->Class != DB_CLASS_DK && lpMsg->Class != DB_CLASS_FE && lpMsg->Class != DB_CLASS_MG && lpMsg->Class != DB_CLASS_DL && lpMsg->Class != DB_CLASS_SU && lpMsg->Class != DB_CLASS_RF)
	{
		DataSend(aIndex, (BYTE*)&pMsg, pMsg.header.size);
		return;
	}
...
#if(GAMESERVER_UPDATE>=1000)//2017/02/28
	if (lpMsg->Class == DB_CLASS_FK && (lpObj->ClassCode & 16) == 0)
	{
		DataSend(aIndex, (BYTE*)&pMsg, pMsg.header.size);
		return;
	}
...
#if(GAMESERVER_UPDATE>=902)//2017/03/02
...
void MakeChatMessagePacket(char* name, char* message, PMSG_CHAT_SEND& pMsg)
{
	int size = strlen(message);
	size = ((size > MAX_CHAT_MESSAGE_SIZE) ? MAX_CHAT_MESSAGE_SIZE : size);
...
void GCChatMessageSend(int aIndex, char* name, char* message) // OK
{
	PMSG_CHAT_SEND pMsg;
	MakeChatMessagePacket(name, message, pMsg);

	DataSend(aIndex, (BYTE*)&pMsg, pMsg.header.size);
}

void GCChatMessageSendAll(char* name, char* message) // OK
{
	PMSG_CHAT_SEND pMsg;
	MakeChatMessagePacket(name, message, pMsg);
...
void MakeChatWhisperMessagePacket(char* name, char* message, PMSG_CHAT_WHISPER_SEND& pMsg)
{
	int size = strlen(message);
	size = ((size > MAX_CHAT_MESSAGE_SIZE) ? MAX_CHAT_MESSAGE_SIZE : size);
...
void GCChatWhisperSend(int aIndex, char* name, char* message) // OK
{
	...
	MakeChatWhisperMessagePacket(name, message, pMsg);

	DataSend(aIndex, (BYTE*)&pMsg, pMsg.header.size);

}

void GCChatWhisperSendAll(char* name, char* message) // OK
{
	PMSG_CHAT_WHISPER_SEND pMsg;
	MakeChatWhisperMessagePacket(name, message, pMsg);
...
void GCDamageSend(int aIndex, int bIndex, BYTE flag, int damage, int type, int ShieldDamage) // OK
{
	...
	if (gObj[aIndex].Type == OBJECT_USER && damage > 0)
	{
		DataSend(aIndex, (BYTE*)&pMsg, pMsg.header.size);
	}

	if (WatchTargetIndex == 99999 || WatchTargetIndex == bIndex || WatchTargetIndex == aIndex || gObj[bIndex].MUFCFighter > 0 || gObj[aIndex].MUFCFighter > 0)
	{
		for (int n = 0; n < MAX_VIEWPORT; n++)
		{
			if (OBJECT_RANGE(gObj[aIndex].VpPlayer2[n].index) == 0)
				continue;

			if (gObj[gObj[aIndex].VpPlayer2[n].index].Type != OBJECT_USER)
				continue;

			if (gObj[gObj[aIndex].VpPlayer2[n].index].Authority == 32)
			{
				DataSend(gObj[aIndex].VpPlayer2[n].index, (BYTE*)&pMsg, pMsg.header.size);
				break;
			}
		}
	}
	...
	if (gObj[bIndex].Type == OBJECT_USER)
	{
		...
		if (gObj[aIndex].Type == OBJECT_MONSTER && damage > 0)
		{
			gCGHuntingGrounds.AddGetNormalDamage(bIndex, aIndex, damage);
		}
	}

	if (gObj[aIndex].Type == OBJECT_USER && gObj[bIndex].Type == OBJECT_MONSTER && damage > 0)
	{
		gCGHuntingGrounds.AddNormalDamage(aIndex, bIndex, damage);

		/*if (g_Labyrinth.IsLabyrinthMapNumber(gObj[aIndex].Map) == 1 && damage > 0)
		{
			gObj[aIndex].m_cLabyrinthInfo->CheckPlayMission(gObj[aIndex].Index, damage, LAB_MISSION_DAMAGE, -1);
		}*/
	}
}
...
void GCMonsterDieSend(int aIndex, int bIndex, int experience, int damage, BYTE flag) // OK
{
	...
	pMsg.damage[0] = SET_NUMBERHB(damage);
	pMsg.damage[1] = SET_NUMBERLB(damage);
...
/*
	...
	/*
	gObj[aIndex].m_cLabyrinthInfo->CheckPlayMission(gObj[aIndex].Index, experience, LAB_MISSION_EXP, -1);
	gObj[aIndex].m_cLabyrinthInfo->AddEntireExp(experience);
	*/
...
void GCElementalDamageSend(int aIndex, int bIndex, BYTE attribute, int damage) // OK
{
	...
	if (gObj[bIndex].Type == OBJECT_USER)
	{
		...
		if (gObj[aIndex].Type == OBJECT_MONSTER && damage > 0)
			gCGHuntingGrounds.AddGetPentagramDamage(bIndex, aIndex, damage);

	}

	if (gObj[bIndex].Type == OBJECT_MONSTER && damage > 0)
		gCGHuntingGrounds.AddPentagramDamage(aIndex, bIndex, damage);
}
...
void GCCharacterRegenSend(LPOBJ lpObj) // OK
{
	...
	/*else
	{
		...
		{
			if (g_InstanceSystemMgr->m_cInstanceObject[i].GetOwnerUserIndex() == lpObj->Index)
			{
				pMsg.Map = g_InstanceSystemMgr->GetInstanceMap(i)->m_MapNumber;
			}
		}
	}*/
...
void GCLevelUpSend(LPOBJ lpObj, bool effect) // OK
{
...
void GCMonsterDamageSend(int aIndex, int damage, int ShieldDamage) // OK
{
	...
	pMsg.damage = GET_MAX_WORD_VALUE(damage);
...
void GCNewCharacterInfoSend(LPOBJ lpObj) // OK
{
	...
	pMsg.ViewMasterReset = (DWORD)(lpObj->MasterReset);
	pMsg.ViewLevel = (DWORD)(lpObj->Level);
	pMsg.ViewMasterLevel = (DWORD)(lpObj->MasterLevel);
	pMsg.ViewPoint = (DWORD)(lpObj->LevelUpPoint);
	pMsg.ViewCurHP = (DWORD)(lpObj->Life);
	pMsg.ViewMaxHP = (DWORD)(lpObj->MaxLife + lpObj->AddLife);
	pMsg.ViewCurMP = (DWORD)(lpObj->Mana);
...
void GCNewCharacterCalcSend(LPOBJ lpObj) // OK
{
	...
	pMsg.ViewAttackSuccessRate = lpObj->m_MPSkillOpt.AttackSuccessRate;
	...
	pMsg.ViewAttackSuccessRatePvP = lpObj->m_MPSkillOpt.AttackSuccessRatePvP;
	gAttack.GetPreviewDefense(lpObj, &pMsg.ViewDefense);
	pMsg.ViewDefenseSuccessRate = lpObj->m_MPSkillOpt.DefenseSuccessRate;
	...
	pMsg.ViewDefenseSuccessRatePvP = lpObj->m_MPSkillOpt.DefenseSuccessRatePvP;
...
void GCNewMessageSend(LPOBJ lpObj, char* message, int type) // OK
{
...
void GCMessagePopupSend(LPOBJ lpObj, char* message, ...)
{
	char buff[1024] = {0};
	...
	int messageSize = strlen(buff);
	messageSize = ((messageSize>MAX_MESSAGE_SIZE)?MAX_MESSAGE_SIZE:messageSize);
	
	PMSG_MESSAGE_POPUP_SEND pMsg;
	pMsg.header.set(0xF3, 0xE2, (sizeof(pMsg)-(sizeof(pMsg.message)-(messageSize+1))));

	memcpy(pMsg.message, buff, messageSize);
	pMsg.message[messageSize] = 0;
...
void GCAnsCsAccessCrownState(int iIndex, BYTE btCrownState)
{
	...
	if (btCrownState == 0)
	{
		if (lpObj->AccumulatedCrownAccessTime < 0 || lpObj->AccumulatedCrownAccessTime > 61000)
		{
			lpObj->AccumulatedCrownAccessTime = 0;
		}

		LogAdd(LOG_BLACK, "[CastleSiege] [Reg. Attempt] Accumulated Crown AccessTime : %d [%s](%s)(%s)", lpObj->AccumulatedCrownAccessTime, lpObj->GuildName, lpObj->Account, lpObj->Name);
	}

	if (btCrownState == 1)
	{
		LogAdd(LOG_BLACK, "[CastleSiege] [Reg. Success] Accumulated Crown AccessTime : %d [%s](%s)(%s)", lpObj->AccumulatedCrownAccessTime, lpObj->GuildName, lpObj->Account, lpObj->Name);
		lpObj->AccumulatedCrownAccessTime = 0;
	}
	...
	if (btCrownState == 2)
	{
		...
		LogAdd(LOG_BLACK, "[CastleSiege] [Reg. Fail] Accumulated Crown AccessTime : %d [%s](%s)(%s)", lpObj->AccumulatedCrownAccessTime, lpObj->GuildName, lpObj->Account, lpObj->Name);
		gCastleSiege.SaveSiegeCharInfo(&gObj[iIndex], 12, gObj[iIndex].AccumulatedCrownAccessTime); //status de tempo acumulado para live
	}

	pMsgResult.dwAccumulatedCrownAccessTime = lpObj->AccumulatedCrownAccessTime;

	DataSend(lpObj->Index, (LPBYTE)&pMsgResult, pMsgResult.h.size);

	//Send accumulated time to guild
	if (lpObj->Guild)
	{
		int iUserIndex = -1;

		//Send accumulated time to direct guild
		if (lpObj->Guild->Count >= 0)
		{
			for (int i = 0; i < MAX_GUILD_USER; i++)
			{
				if (lpObj->Guild->Use[i] != FALSE)
				{
					iUserIndex = lpObj->Guild->Index[i];

					if (iUserIndex >= 0 && lpObj->Guild->Names[i][0] == gObj[iUserIndex].Name[0] && !strcmp(lpObj->Guild->Names[i], gObj[iUserIndex].Name))
					{
						DataSend(iUserIndex, (LPBYTE)&pMsgResult, pMsgResult.h.size);
					}
				}
			}
		}
		...
		//Send accumulated time to all alliance
		if (gUnionManager.GetGuildUnionMemberList(lpObj->GuildNumber, iGuildCount, iGuildList) == TRUE)
		{
			iUserIndex = -1;
...
void Countdown(int iIndex, int type, int unk, DWORD times)
{
#if(GAMESERVER_UPDATE>=1220)//2018/04/15
	PMSG_COUNT_TIME_SEND pMsg;

	pMsg.header.setE(0xA5, 0x01, sizeof(pMsg));
...
void CGCharRecv(PMSG_CHATDATA_NUMBER* lpMsg, int aIndex)
{
	LPOBJ lpObj = &gObj[aIndex];
	DataSend(aIndex, (BYTE*)&lpMsg, lpMsg->h.size);
	MsgSendV2(lpObj, (BYTE*)&lpMsg, lpMsg->h.size);
}

void  CGReqDoUsePopupType(PMSG_REQ_USE_POPUP* lpMsg, int aIndex)
{
...
void GCShutdownThat_dd_55_Shit(int aIndex)
{
	PMSG_DD55 pMsg;
	pMsg.h.set(0x79, 0x00, sizeof(pMsg));
	pMsg.unk1 = 0;
	pMsg.unk2 = 0;
	pMsg.time = 0;
