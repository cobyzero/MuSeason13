...
{
	int IndexTable[MAX_SET_ITEM_TYPE] = { 0 };
...
#if(GAMESERVER_UPDATE>=902)//2017/03/07
void CSetItemType::GremoryCaseMakeRandomSetItem(int aIndex,int map,int x,int y,BYTE btStorageType,BYTE btRewardSource,int iDaysToExpire) // OK
{
	WORD ItemIndex = this->GetRandomSetItem();
	...
	gItemOptionRate.MakeSocketOption(ItemIndex,ItemSocketOption[0],&ItemSocketOption[0]);
		...
		stItem.ItemInfo.m_Index =ItemIndex;
		stItem.ItemInfo.m_Level = ItemLevel;
		stItem.ItemInfo.m_Option1 = ItemOption1;
		stItem.ItemInfo.m_Option2 = ItemOption2;
		stItem.ItemInfo.m_Option3 = ItemOption3;
		stItem.ItemInfo.m_NewOption = ItemNewOption;
		stItem.ItemInfo.m_SetOption = ItemSetOption;
		memcpy(stItem.ItemInfo.m_SocketOption,ItemSocketOption,sizeof(ItemSocketOption));
		g_GremoryCase.GDReqAddItemToGremoryCase(aIndex, stItem, iDaysToExpire);
	//GDCreateItemSend(aIndex,map,x,y,ItemIndex,ItemLevel,0,ItemOption1,ItemOption2,ItemOption3,aIndex,ItemNewOption,ItemSetOption,0,0,ItemSocketOption,0xFF,0);
}
#endif