...
bool CNpcTalk::NpcTalk(LPOBJ lpNpc,LPOBJ lpObj) // OK
{
	...
	switch(lpNpc->Class)
	{
		...
		case 724:
			...
			break;
		case 725:
			this->NpcRheaGold(lpNpc,lpObj);
			break;
		case 764:
		case 765:
		//case 766:
			this->NPCCent(lpNpc, lpObj);
			break;
		default:
			return 0;
	}
...
void CNpcTalk::NpcCastleSiegeCrownSwitch(LPOBJ lpNpc,LPOBJ lpObj) // OK
{
	...
	{
		gCastleSiege.SaveSiegeCharInfo(lpObj, (lpNpc->Class - 211));
...
void CNpcTalk::NpcWarehouse(LPOBJ lpNpc,LPOBJ lpObj) // OK
{
	...
	lpObj->WarehouseNumber = gWarehouse.CheckNPCVaultNumber(lpNpc);
...
void CNpcTalk::NpcRheaGold(LPOBJ lpNpc,LPOBJ lpObj) // OK
{
	PMSG_NPC_TALK_SEND pMsg;
	...
	DataSend(lpObj->Index,(BYTE*)&pMsg,pMsg.header.size);
	gRheaGold.RheaGoldItemList(lpObj);

}
void CNpcTalk::NpcMonica(LPOBJ lpNpc,LPOBJ lpObj) // OK
{
	PMSG_NPC_TALK_SEND pMsg;
...
void CNpcTalk::NPCCent(LPOBJ lpNpc, LPOBJ lpObj) // OK
{
	gFourthQuest.CentTalk(lpNpc, lpObj);
	GCTaxInfoSend(lpObj->Index, 1, gCastleSiegeSync.GetTaxRateChaos(lpObj->Index));
}