...
bool CCustomJewel::CheckCustomJewelApplyItem(int ItemIndex,CItem* lpItem) // OK
{
	...
	if(lpItem->m_Index > GET_ITEM(12,6) && (lpItem->m_Index < GET_ITEM(12,36) || lpItem->m_Index > GET_ITEM(12,43)) && lpItem->m_Index != GET_ITEM(12,49) && lpItem->m_Index != GET_ITEM(12,50) && (lpItem->m_Index < GET_ITEM(12,262) || lpItem->m_Index > GET_ITEM(12,270)) && lpItem->m_Index != GET_ITEM(13,30) && gCustomWing.CheckCustomWingByItem(lpItem->m_Index) == 0)
#else
	...
	if(lpInfo->MaxItemOption1 != -1 && lpItem->m_Option1 > lpInfo->MaxItemOption1)
	{
		return 0;
	}
...
bool CCustomJewel::CheckCustomJewelApplyItemSocketOption(CUSTOM_JEWEL_INFO* lpInfo,CItem* lpItem) // OK
{
	...
	if(lpInfo->MaxItemSocketOption != -1 && ((lpInfo->MaxItemSocketOption == 0 && gSocketItemType.CheckSocketItemType(lpItem->m_Index) != 0) || (lpInfo->MaxItemSocketOption != 0 && (gSocketItemType.CheckSocketItemType(lpItem->m_Index) == 0 || GetSocketOptionCount(lpItem->m_SocketOption) > lpInfo->MaxItemSocketOption))))
	{
		return 0;
	}
...
int CCustomJewel::GetCustomJewelNewOption(CItem* lpItem,int value) // OK
{
	...
	if(
		#if(GAMESERVER_UPDATE>=1000)//2017/03/10
		lpItem->m_Index == GET_ITEM(12,269) ||
#endif
		(lpItem->m_Index >= GET_ITEM(12,3) && lpItem->m_Index <= GET_ITEM(12,6)) || lpItem->m_Index == GET_ITEM(12,42) || lpItem->m_Index == GET_ITEM(12,49) || lpItem->m_Index == GET_ITEM(13,30)) // 2sd Wings
	{
	...
	if(
		#if(GAMESERVER_UPDATE>=1000)//2017/03/10
		lpItem->m_Index == GET_ITEM(12,270)||
#endif
