...
void gObjMonsterDieGiveItem(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	...
	if (lpObj->Class == 746) // NIX
	{
		int UserCnt = g_NixiesLakeBossZone.GetBossZonePlayerCnt();
...
#if IAJulia 1
	if (DropSystem.DropItem(lpObj, lpTarget))
	{
		return;
	} 
#else
...
#endif
	if (lpObj->Money > 0 && (GetLargeRand() % ((lpObj->MoneyRate == 0) ? 1 : lpObj->MoneyRate)) < 10)
	{
		...
		if (lpTarget->Map == MAP_Labyrinth)
		{
			gMap[lpObj->Map].LabyrinthMoneyItemDrop(lpTarget->Index, money, lpObj->X, lpObj->Y);
		}
		else
		{
	...
	if((DropIndex=gMonsterManager.GetMonsterItem(DropLevel,ExcItemDrop,gMapManager.GetMapSocketItemDrop(lpObj->Map))) >= GET_ITEM(0,0) && (GetLargeRand()%((lpObj->ItemRate==0)?1:lpObj->ItemRate)) < ItemDropRate)
	{
		...
		gLog.Output(LOG_ITEM_DROP, "[MonsterDie][%s][%s] [Map: %d] [Monster: %s] [Item: %s%s +%d] [Ancient: %d] [Socket: %d %d %d %d %d]", lpTarget->Account, lpTarget->Name, lpObj->Map, lpObj->Name, (ItemNewOption > 0) ? "Excellent " : "", gItemManager.GetName(ItemIndex), ItemLevel, ItemSetOption, ItemSocketOption[0], ItemSocketOption[1], ItemSocketOption[2], ItemSocketOption[3], ItemSocketOption[4]);
...
bool gObjSetPosMonster(int aIndex,int number) // OK
{
	...
	lpObj->OldX = lpObj->X;
	lpObj->OldY = lpObj->Y;
	lpObj->SpawnX = lpObj->X;
	lpObj->SpawnY = lpObj->Y;
...
bool gObjSetMonster(int aIndex,int MonsterClass, bool DeleteAfterDie, bool RespawnAfterDie) // OK
{
	if(OBJECT_RANGE(aIndex) == 0)
	{
		return 0;
	...
	lpObj->DeleteAfterDie = DeleteAfterDie;
	lpObj->RespawnAfterDie = RespawnAfterDie;
	...
	lpObj->m_MPSkillOpt.AttackSuccessRate = lpInfo->AttackRate;
	lpObj->m_MPSkillOpt.DefenseSuccessRate = lpInfo->DefenseRate;
	...
	lpObj->m_MPSkillOpt.Resistance[0] = ((lpInfo->Resistance[0]>255)?255:lpInfo->Resistance[0]);
	lpObj->m_MPSkillOpt.Resistance[1] = ((lpInfo->Resistance[0]>255)?255:lpInfo->Resistance[1]);
	lpObj->m_MPSkillOpt.Resistance[2] = ((lpInfo->Resistance[0]>255)?255:lpInfo->Resistance[2]);
	lpObj->m_MPSkillOpt.Resistance[3] = ((lpInfo->Resistance[0]>255)?255:lpInfo->Resistance[3]);
	lpObj->m_MPSkillOpt.Resistance[4] = ((lpInfo->Resistance[0]>255)?255:lpInfo->Resistance[4]);
	lpObj->m_MPSkillOpt.Resistance[5] = ((lpInfo->Resistance[0]>255)?255:lpInfo->Resistance[5]);
	lpObj->m_MPSkillOpt.Resistance[6] = ((lpInfo->Resistance[0]>255)?255:lpInfo->Resistance[6]);
	...
	if(lpObj->AttackType != 0)
	{
		...
		if(MonsterClass == 673)
		{
			gSkillManager.AddSkill(lpObj,622,0);
		...
		{
			gSkillManager.AddSkill(lpObj,SKILL_EVIL_SPIRIT,0);
			gSkillManager.AddSkill(lpObj,SKILL_HELL_FIRE,0);
			gSkillManager.AddSkill(lpObj,SKILL_POISON ,0);
		}
	}
...
bool gObjMonsterRegen(LPOBJ lpObj) // OK
{
	...
	//if (lpObj->Map < INST_MAP_INDEX_LABYRINTH)
	//{
		...
		{
			if (lpObj->RespawnAfterDie)
			{
				lpObj->X = lpObj->SpawnX;
				lpObj->Y = lpObj->SpawnY;
			}
			else if (gMonsterSetBase.GetPosition(lpObj->PosNum, lpObj->Map, &lpObj->X, &lpObj->Y) == 0)
			{
				lpObj->Live = 0;
				lpObj->State = OBJECT_DYING;
				lpObj->DieRegen = 1;
...
int gObjMonsterGetTopHitDamageUser(LPOBJ lpMonObj, int quant)
{
	int sortedCount = 0;

	for (int i = 0; i < MAX_HIT_DAMAGE; i++)
	{
		HIT_DAMAGE_STRUCT* firstPlayer = &lpMonObj->HitDamage[i];

		if (lpMonObj->HitDamage[i].index < 0 || (gObjIsConnected(lpMonObj->HitDamage[i].index) == 0))
		{
			HIT_DAMAGE_STRUCT temp;

			temp.damage = firstPlayer->damage;
			temp.index	= firstPlayer->index;
			temp.time	= firstPlayer->time;

			for (int j = i; j < MAX_HIT_DAMAGE - 1; j++)
			{
				lpMonObj->HitDamage[j].damage = lpMonObj->HitDamage[j + 1].damage;
				lpMonObj->HitDamage[j].index = lpMonObj->HitDamage[j + 1].index;
				lpMonObj->HitDamage[j].time = lpMonObj->HitDamage[j + 1].time;
			}

			lpMonObj->HitDamage[MAX_HIT_DAMAGE - 1].damage = temp.damage;
			lpMonObj->HitDamage[MAX_HIT_DAMAGE - 1].index = temp.index;
			lpMonObj->HitDamage[MAX_HIT_DAMAGE - 1].time = temp.time;

			continue;
		}

		for (int j = i; j < MAX_HIT_DAMAGE; j++)
		{
			if (lpMonObj->HitDamage[j].index < 0 || (gObjIsConnected(lpMonObj->HitDamage[i].index) == 0))
			{
				continue;
			}

			HIT_DAMAGE_STRUCT* secondPlayer = &lpMonObj->HitDamage[j];

			if (firstPlayer->damage < secondPlayer->damage)
			{
				HIT_DAMAGE_STRUCT temp;
				...
				firstPlayer->damage = secondPlayer->damage;
				firstPlayer->index = secondPlayer->index;
				firstPlayer->time = secondPlayer->time;

				secondPlayer->damage = temp.damage;
				secondPlayer->index = temp.index;
				secondPlayer->time = temp.time;
			}
		}
	...
	return sortedCount <= quant ? sortedCount : quant;
}
...
void gObjMonsterMagicAttack(LPOBJ lpObj,int skill) // OK
{
	...
	if(/*lpObj->Class == 707 ||*/lpObj->Class == 161 || lpObj->Class == 181 || lpObj->Class == 189 || lpObj->Class == 197 || lpObj->Class == 267 || lpObj->Class == 275 || lpObj->Class == 338)
	{
...
void gObjMonsterAttack(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	...
	else if(lpObj->Class == 707 ||lpObj->Class == 161 || lpObj->Class == 181 || lpObj->Class == 189 || lpObj->Class == 197 || lpObj->Class == 267 || lpObj->Class == 275 || lpObj->Class == 338)
	{
		PMSG_SKILL_ATTACK_RECV pMsg;
	...
	else if(lpObj->Class == 673)
	{
		
		/*BYTE i70,i50,i20;
		if(lpObj->Life > lpObj->Life/100*70)
		{
			i70=0;
			i50=0;
			i20=0;
		}
		if(lpObj->Life<=lpObj->Life/100*70 &&i70==0)
		{
			i70 = 1;
			pMsg.skillL =5;
		gSkillManager.CGSkillAttackRecv(&pMsg,lpObj->Index);
		...
		if(lpObj->Life<=lpObj->Life/100*50 &&i50==0)
		{
			i50 = 1;
			pMsg.skillL =5;
		gSkillManager.CGSkillAttackRecv(&pMsg,lpObj->Index);
		...
		if(lpObj->Life<=lpObj->Life/100*20 &&i20==0)
		{
			i20 = 1;
			pMsg.skillL =5;
		gSkillManager.CGSkillAttackRecv(&pMsg,lpObj->Index);
...
void gObjMonsterDie(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	...
	if (lpObj->Class == 747)
	{
		LogAdd(LOG_GREEN, gMessage.GetMessage(537), lpObj);
		g_NixiesLakeBoss.RemoveRestrictStone(lpObj);
	}

	if (lpObj->Class == 746)
	{
		LogAdd(LOG_GREEN, gMessage.GetMessage(538), lpObj);
		g_NixiesLakeBoss.DieNixiesBoss();
	}
...
void gObjMonsterStateProc(LPOBJ lpObj,int code,int aIndex,int SubCode) // OK
{
	switch(code)
	{
		...
		case 57:
			...
			break;
#if(GAMESERVER_UPDATE>=1200)//2018/05/25
		case 74:
			g_NixiesLakeBoss.IceBoomAttackDamage(lpObj);
			break;
		case 75:
			g_NixiesLakeBoss.DevilIceAttackDamage(lpObj);
			break;
#endif
	}
}
...
int gObjMonsterSearchEnemy(LPOBJ lpObj, BYTE objtype)
{
	...
	for (n=0;n<MAX_VIEWPORT;n++)
	{
		...
		if ( tObjNum >= 0 )
		{
			if ( gObj[tObjNum].Type == objtype && (gObj[tObjNum].Live != FALSE) )
			{
				if ( ((gObj[tObjNum].Class >= 100 && gObj[tObjNum].Class < 110 ) || gObj[tObjNum].Class == 523) // TARGET TRAPS
					|| (gObj[tObjNum].Type == OBJECT_MONSTER && gObj[tObjNum].SummonIndex >= 0) ) // TARGET SUMMONS
				{
					continue;
				}

				if ( (gObj[tObjNum].Authority &2) != 2 && (gObj[tObjNum].Authority &32) != 32 && gObj[tObjNum].Teleport == 0 )
				{
					if (!gFourthQuest.ShouldSearchTarget(lpObj, &gObj[tObjNum], objtype)) continue;
...
void gObjMonsterProcess(LPOBJ lpObj)
{
	...
	if (lpObj->Class == 746)
	{
		if (!g_NixiesLakeBossZone.GetBossZoneState())
		{
			if (g_NixiesLakeBossZone.ChkBossZonePlayer() == 1)
				g_NixiesLakeBossZone.SetBossZoneState(1);
			return;
		}
		if (g_NixiesLakeBossZone.GetBossZoneState() == 2)
		{
			LogAdd(LOG_BLACK, gMessage.GetMessage(539));
			return;
		}
		if (lpObj->VPCount < 1)
		{
			if (g_NixiesLakeBoss.ChkRefillLifeDelay() == 1)
			{
				if (g_NixiesLakeBossZone.ChkBossZonePlayer())
				{
					g_NixiesLakeBoss.RefillLife( lpObj);
					g_NixiesLakeBoss.DisableCountSkill();
				}
				else
				{
					g_NixiesLakeBoss.FailBossBattles(lpObj);
				}
			}
			return;
		}
		g_NixiesLakeBoss.SetRefillLifeDelay();
		if (g_NixiesLakeBoss.NixiesBossSkillRun(lpObj) == 1)
			return;
	}
#endif
	...
	if ( lpObj->ActionState.Attack == 1 )
	{
		...
		if ( CMonsterSkillManager::CheckMonsterSkill(lpObj->Class,0) )
		{
			...
			if (lpObj->Class ==746)
			{
				g_NixiesLakeBoss.NixAttack(lpObj);
				lpObj->ActionState.Attack = 0;
				return;
			}
			if ( GetLargeRand()%4 == 0 )
...
void gObjMonsterBaseAct(LPOBJ lpObj)
{
	...
	{
#if(GAMESERVER_UPDATE>=901)//2017/03/10
		if ( lpObj->Class == 673 )
		{
