...
void DataServerProtocolCore(BYTE head, BYTE* lpMsg, int size) // OK
{
	PROTECT_START
		...
		case 0x29:
			...
			break;
		case 0x32:
			DGGlobalPopupRecv((SDHP_GLOBAL_POPUP_RECV*)lpMsg);
			break;
		case 0x4F:
		{
		...
		case 0x80:
			...
			case 0x1B:
				...
				break;
			case 0xA1:
				GS_DGLoadSiegeCharInfo((SEND_CHAR_SIEGE_INFO*)lpMsg);
				break;
			}
			break;
		case 0x81:
...
void DGCreateItemRecv(SDHP_CREATE_ITEM_RECV* lpMsg) // OK
{
	...
	if (lpMsg->Map == 0xFE || lpMsg->Map == 0xFF)
	{
		...
		if (lpMsg->ItemIndex == GET_ITEM(13, 18) || lpMsg->ItemIndex == GET_ITEM(13, 37) || lpMsg->ItemIndex == GET_ITEM(13, 51) || lpMsg->ItemIndex == GET_ITEM(14, 7) || lpMsg->ItemIndex == GET_ITEM(14, 19) || lpMsg->ItemIndex == GET_ITEM(14, 109) || lpMsg->ItemIndex == GET_ITEM(12, 404) || lpMsg->ItemIndex == GET_ITEM(12, 405) || lpMsg->ItemIndex == GET_ITEM(12, 406))
		{
...
void DGGlobalPostRecv(SDHP_GLOBAL_POST_RECV* lpMsg) // OK
{
	...
	PostMessage(lpMsg->name, gMessage.GetMessage(69), lpMsg->message, lpMsg->type);
}
...
void DGGlobalPopupRecv(SDHP_GLOBAL_POPUP_RECV* lpMsg) // OK
{
	if (gMapServerManager.GetMapServerGroup() != lpMsg->MapServerGroup)
	{
		return;
	...
	gNotice.GCPopupSendToAll(lpMsg->message);
}
...
void GDGlobalPopupSend(int MapServerGroup, char* message) // OK
{
	SDHP_GLOBAL_NOTICE_SEND pMsg;
...
void GDCharacterInfoSaveSend(int aIndex) // OK
{
	...
	for(int i=0;i<5;i++)
		pMsg.AutoDt[i] = lpObj->AutoDt[i];

	for (int i=0; i<50; i++)
	{
		const auto questNumber = i*4;

		pMsg.Quest[i] = lpObj->Quest[questNumber].questState | lpObj->Quest[questNumber + 1].questState << 2
...
void GDPkInfoSaveSend(int aIndex) // OK
{
	LPOBJ lpObj = &gObj[aIndex];

	SDHP_PK_INFO_SAVE_SEND pMsg;
...
void GDCustomMonsterRewardSaveSend(int aIndex, int MonsterClass, int MapNumber, int RewardValue1, int RewardValue2, int RewardValue3, int damage) // OK
{
...
void GS_DGLoadSiegeCharInfo(SEND_CHAR_SIEGE_INFO* lpRecv)
{
	if (!OBJECT_RANGE(lpRecv->aIndex))
	{
		return;
	}

	LPOBJ lpObj = &gObj[lpRecv->aIndex];
	...
	lpObj->SiegeTotalKillCount = lpRecv->SiegeTotalKillCount;
	lpObj->SiegeTotalDeathCount = lpRecv->SiegeTotalDeathCount;
}