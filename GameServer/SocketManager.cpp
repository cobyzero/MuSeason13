		...
		if(size <= lpIoBuffer->size)
		{
			if(header == 0xC3 || header == 0xC4)
			{
				if(header == 0xC3)
				{
					...
					{
						//LogAdd(LOG_RED,"AddData or Extract1");
						return 0;
					}
				...
				else
				{
					...
					{
						//LogAdd(LOG_RED,"AddData or Extract2");
						return 0;
					}
			...
			{
#if (GAMESERVER_UPDATE==1001)
				if(DecBuff[3]==0x0E&&DecBuff[2]==0xF3)
				{
					if(gPacketManager.AddData(DecBuff,size) == 0 || gPacketManager.ExtractPacket(DecBuff) == 0)
					{
						return 0;
					}
				...
				else
				{
					...
					{
						//LogAdd(LOG_RED,"AddData or Extract3");
						return 0;
					}
...
void CSocketManager::OnRecv(int index,DWORD IoSize,IO_RECV_CONTEXT* lpIoContext) // OK
{
	...
	if(IoSize == 0)
	{
		//LogAdd(LOG_RED,"OnRecv Disconnect");
		this->Disconnect(index);
		this->m_critical.unlock();
		return;
	}
	...
	{
		//LogAdd(LOG_RED,"OnRecv Disconnect 2");
		this->Disconnect(index);
		this->m_critical.unlock();
		return;
	}
...
void CSocketManager::OnSend(int index,DWORD IoSize,IO_SEND_CONTEXT* lpIoContext) // OK
{
	...
	if(IoSize == 0)
	{
		//LogAdd(LOG_RED,"OnSend Disconnect");
		this->Disconnect(index);
		this->m_critical.unlock();
		return;
	}
...
DWORD WINAPI CSocketManager::ServerAcceptThread(CSocketManager* lpSocketManager) // OK
{
	...
	while(true)
	{
		//SOCKET socket = WSAAccept(lpSocketManager->m_listen,(sockaddr*)&SocketAddr,&SocketAddrSize,&lpSocketManager->ServerAcceptCondition,(DWORD)lpSocketManager);
		SOCKET socket = WSAAccept(lpSocketManager->m_listen, (sockaddr*)&SocketAddr, &SocketAddrSize,NULL, 0);
