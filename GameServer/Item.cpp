...
void CItem::Convert(int index,BYTE Option1,BYTE Option2,BYTE Option3,BYTE NewOption,BYTE SetOption,BYTE JewelOfHarmonyOption,BYTE ItemOptionEx,BYTE SocketOption[MAX_SOCKET_OPTION],BYTE SocketOptionBonus) // OK
{
	...
	this->m_IsLocked = ((SetOption & 128) / 128) & 1;
	...
	if(this->IsSocketItem() != 0 /*&& gMasteryItemExtOptionSet.IsMasteryItem(index) == 0*/)
	{
	...
	if(ItemInfo.RequireLevel != 0)
	{
		...
		else if(this->m_Index == GET_ITEM(12,270) || (this->m_Index >= GET_ITEM(12,36) && this->m_Index <= GET_ITEM(12,40)) || this->m_Index == GET_ITEM(12,43) || this->m_Index == GET_ITEM(12,50)) // 3rd Wings
		{
			this->m_RequireLevel = ItemInfo.RequireLevel;
		}
		...
		else if (this->m_Index >= GET_ITEM(12, 414) && this->m_Index <= GET_ITEM(12, 421)) // 4th Wings
		{
			this->m_RequireLevel = ItemInfo.RequireLevel;
		}
	...
	if(this->m_Index == GET_ITEM(13,269) || this->m_Index == GET_ITEM(13,30) || this->m_Index == GET_ITEM(12,49)) // Cloak of Lord,Cloak of Fighter
	{
		this->m_Defense = 15;
	}
	...
	if(this->m_Defense > 0)
	{
		...
		else
		{
			...
			else if(this->m_Index == GET_ITEM(12,270) ||
				   (this->m_Index >= GET_ITEM(12,36) && this->m_Index <= GET_ITEM(12,40)) || this->m_Index == GET_ITEM(12,43) || this->m_Index == GET_ITEM(12,50)) // 3rd Wings
			{
	...
	else
	{
		...
		switch(this->m_Index)
		{
			...
			case GET_ITEM(12,269): // Cloak of hyesung
				this->m_RequireStrength += (this->m_Option3*4);
				break;
#endif
			default:
...
void CItem::Value() // OK
{
	...
	else
	{
		...
		#if(GAMESERVER_UPDATE>=1000)//2017/03/10
		if(((this->m_Index/MAX_ITEM_TYPE) == 12 && ((this->m_Index > GET_ITEM(12,6) && this->m_Index < GET_ITEM(12,36)) || this->m_Index > GET_ITEM(12,43)) && this->m_Index != GET_ITEM(12,50) && (this->m_Index < GET_ITEM(12,262) || this->m_Index > GET_ITEM(12,270)) && gCustomWing.CheckCustomWingByItem(this->m_Index) == 0) || (this->m_Index/MAX_ITEM_TYPE) == 13 || (this->m_Index/MAX_ITEM_TYPE) == 15)
#else
		...
		else
		{
			...
			if((this->m_Index >= GET_ITEM(12,0) && this->m_Index <= GET_ITEM(12,6)) || (this->m_Index >= GET_ITEM(12,36) && this->m_Index <= GET_ITEM(12,43)) || this->m_Index == GET_ITEM(12,50) || (this->m_Index >= GET_ITEM(12,262) && this->m_Index <= GET_ITEM(12,270)) || gCustomWing.CheckCustomWingByItem(this->m_Index) != 0) // Wings
			{
...
bool CItem::WeaponDurabilityDown(int aIndex,int defense,int type) // OK
{
	...
	MaxDurSmall = (MaxDurSmall*gObj[aIndex].m_MPSkillOpt.WeaponDurabilityRate)/100;
...
bool CItem::ArmorDurabilityDown(int aIndex,int damage) // OK
{
	...
	else if(ItemInfo.RequireClass[CLASS_GL] != 0)
	{
		defense *= 3;
	...
	MaxDurSmall = (MaxDurSmall*gObj[aIndex].m_MPSkillOpt.ArmorDurabilityRate)/100;
...
bool CItem::PendantDurabilityDown(int aIndex,int decrease) // OK
{
	...
	MaxDurSmall = (MaxDurSmall*gObj[aIndex].m_MPSkillOpt.PendantDurabilityRate)/100;
...
bool CItem::RingDurabilityDown(int aIndex,int decrease) // OK
{
	...
	MaxDurSmall = (MaxDurSmall*gObj[aIndex].m_MPSkillOpt.RingDurabilityRate)/100;
