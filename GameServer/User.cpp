...
#endif
	gRaklion.MainProc();
#if(GAMESERVER_UPDATE>=1100)//2017/04/18
	//gCGHuntingGrounds.MainProc();
#endif
#if(GAMESERVER_UPDATE>=1200)//2018/01/07
...
void gObjCharZeroSet(int aIndex) // OK
{
	...
	lpObj->m_btInvenPetDkSpiritPos = -1;

	lpObj->m_btInvenPetPos =-1;
...
void gObjClearPlayerOption(LPOBJ lpObj) // OK
{
	...
	//for(int n=0;n<MAX_NEW_QUEST_WORLD_LIST;n++)
	//{
	//	memset(&lpObj->NewQuestWorldList[n],0xFF,sizeof(lpObj->NewQuestWorldList[n]));
	//}
	memset(lpObj->NewQuestWorldList,0xFF,sizeof(lpObj->NewQuestWorldList));
#endif
#if(GAMESERVER_UPDATE>=1220)//2018/04/19
...
BOOL gObjMoveGate(int aIndex,int gate) // OK
{
	...
	if(TargetGate == 97)
	{
		...
		{
			if (lpObj->Guild && lpObj->GuildStatus != GUILD_MASTER)
			{
				int membersInside = 0;
				...
				{
					gNotice.GCNoticeSend(aIndex, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(37), lpObj->Guild->membersInTower, gServerInfo.m_CastleSiegeMaxMembersInTower);
					goto ERROR_JUMP;
				}
			}
	...
	for(n = 0; n < MAX_OBJECT;n++)
	{
		...
		if(lpObj->Connected > OBJECT_LOGGED)
		{
			...
			if(lpObj->Type == OBJECT_USER)
			{
				...
				if (lpObj->Map == MAP_LAND_OF_TRIALS && gCastleSiege.CheckCastleOwnerUnionMember(lpObj->Index) == FALSE && lpObj->Authority != 32)
				{
		...
		{
			if(GetTickCount() - lpObj->AutoSaveTime > 60000)
			{
