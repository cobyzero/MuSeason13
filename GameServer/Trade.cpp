...
void CTrade::CGTradeRequestRecv(PMSG_TRADE_REQUEST_RECV* lpMsg, int aIndex) // OK
{
	...
	if (CA_MAP_RANGE(lpTarget->Map) != 0 || 
		DS_MAP_RANGE(lpTarget->Map) != 0 || 
		BC_MAP_RANGE(lpTarget->Map) != 0 || 
		CC_MAP_RANGE(lpTarget->Map) != 0 || 
		IT_MAP_RANGE(lpTarget->Map) != 0 || 
		DA_MAP_RANGE(lpTarget->Map) != 0 || 
		DG_MAP_RANGE(lpTarget->Map) != 0 || 
		IG_MAP_RANGE(lpTarget->Map) != 0 || 
#if(GAMESERVER_TYPE==1)		
		(gCastleSiege.GetCastleState() == CASTLESIEGE_STATE_STARTSIEGE && gCastleSiege.IsInDragonTower(lpTarget)) ||
#endif
		lpObj->isGhost)
	{
...
void CTrade::CGTradeOkButtonRecv(PMSG_TRADE_OK_BUTTON_RECV* lpMsg, int aIndex) // OK
{
	...
	/*if (gDuel.CheckDuel(lpObj, lpTarget))
	{
		gObjInventoryCommit(aIndex);
		gObjInventoryCommit(bIndex);
