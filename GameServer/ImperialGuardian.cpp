...
void CImperialGuardian::SetSideGate() // OK
{
	...
	lpObj->m_MPSkillOpt.AttackSuccessRate = (int)this->GetMonsterAttackSuccessRate((float)lpObj->m_MPSkillOpt.AttackSuccessRate);
	lpObj->m_MPSkillOpt.DefenseSuccessRate = (int)this->GetMonsterDefenseSuccessRate((float)lpObj->m_MPSkillOpt.DefenseSuccessRate);
...
void CImperialGuardian::DropSideBossItem(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	...
	{
	case 0:
		bagIndex = ITEM_BAG_IMPERIAL_GUARDIAN_SIDE_BOSS1;
		break;
	case 1:
		bagIndex = ITEM_BAG_IMPERIAL_GUARDIAN_SIDE_BOSS2;
		break;
	case 2:
		bagIndex = ITEM_BAG_IMPERIAL_GUARDIAN_SIDE_BOSS3;
		break;
	case 3:
		bagIndex = ITEM_BAG_IMPERIAL_GUARDIAN_SIDE_BOSS4;
		break;
	default:
		bagIndex = ITEM_BAG_IMPERIAL_GUARDIAN_SIDE_BOSS5;
		break;
	}
	...
	for(int n=0;n < MAX_IG_USER;n++)
	{
		if(OBJECT_RANGE(this->m_User[n].Index) != 0)
		{
			int userIndex = this->m_User[n].Index;

			if (this->m_DayOfWeek != 0)
			{
				GDCreateItemSend(userIndex, gObj[userIndex].Map, (BYTE)gObj[userIndex].X, (BYTE)gObj[userIndex].Y, (GET_ITEM(14, 102) + this->m_DayOfWeek), 0, 0, 0, 0, 0, this->m_User[n].Index, 0, 0, 0, 0, 0, 0xFF, 0);
			}

			gItemBagManager.GremoryCaseDropItemBySpecialValue(bagIndex, GC_STORAGE_CHARACTER, GC_REWARD_VARKA_MAP, &gObj[userIndex], gObj[userIndex].Map, gObj[userIndex].X, gObj[userIndex].Y);
		}
	}
}
...
void CImperialGuardian::DropMainBossItem(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	...
	{
		case 0:
			bagIndex = ITEM_BAG_IMPERIAL_GUARDIAN_MAIN_BOSS1;
			break;
		case 1:
			bagIndex = ITEM_BAG_IMPERIAL_GUARDIAN_MAIN_BOSS2;
			break;
		case 2:
			bagIndex = ITEM_BAG_IMPERIAL_GUARDIAN_MAIN_BOSS3;
			break;
		case 3:
			bagIndex = ITEM_BAG_IMPERIAL_GUARDIAN_MAIN_BOSS4;
			break;
		default:
			bagIndex = ITEM_BAG_IMPERIAL_GUARDIAN_MAIN_BOSS5;
			break;
	}
