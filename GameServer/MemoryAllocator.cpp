...
void CMemoryAllocator::BindMemoryAllocatorInfo(int index,CMemoryAllocatorInfo MemoryAllocatorInfo) // OK
{
	...
	if(OBJECT_USER_RANGE(index) != 0)
	{
		...
		lpObj->MuunInventory1 = MemoryAllocatorInfo.m_MuunInventory1;
		lpObj->MuunInventory2 = MemoryAllocatorInfo.m_MuunInventory2;
		...
		lpObj->m_pCBombHuntInfo = MemoryAllocatorInfo.m_pCBombHuntInfo;
		lpObj->NewQuestWorldList = MemoryAllocatorInfo.m_NewQuestWorldList;
		lpObj->ForMasterSkill = MemoryAllocatorInfo.m_ForMasterSkill;
		lpObj->m_cLabyrinthInfo = MemoryAllocatorInfo.m_LabyrinthInfo;
		lpObj->pRestoreInventory = MemoryAllocatorInfo.m_pRestoreInventory;
		lpObj->pRestoreInventory1 = MemoryAllocatorInfo.m_pRestoreInventory1;
	}
	else
	{
