...
#define PROTOCOL_CODE2 0xDF
#define PROTOCOL_CODE3 0x10
#define PROTOCOL_CODE4 0xD3
...
struct PMSG_TIME_ICON_SEND
{
	//0xC1,0x06,0xF1,0xFE,0x07,0x01
...
/*
struct PMSG_CHARACTER_LIST_SEND  //S13��ɫ�б�
{
...
struct PMSG_MESSAGE_POPUP_SEND
{
	PSBMSG_HEAD header; // C1:F3:E2
	char message[1024];
};
...
void GCChatMessageSend(int aIndex,char* name,char* message);
void GCChatMessageSendAll(char* name, char* message);
void GCChatWhisperSend(int aIndex,char* name,char* message);
void GCChatWhisperSendAll(char* name, char* message);
...
void GCLevelUpSend(LPOBJ lpObj, bool effect=true);
...
void GCNewMessageSend(LPOBJ lpObj,char* message, int type);
