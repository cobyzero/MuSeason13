...
void CCrywolf::SetCrywolfCommonNPC(int state) // OK
{
	for(int n=0;n < this->m_ObjCommonNPC.m_ObjCount;n++)
	{
		...
		if(state == 1 || state == 2)
		{
			gEffectManager.AddEffect(&gObj[this->m_ObjCommonNPC.m_ObjIndex[n]],0,EFFECT_CRYWOLF_NPC_TRANSPARENCY,0,0,0,0,0);
		}
		else if(state == 0)
		{
			gEffectManager.DelEffect(&gObj[this->m_ObjCommonNPC.m_ObjIndex[n]],EFFECT_CRYWOLF_NPC_TRANSPARENCY);
		}
	}
}
