		...
		while(true)
		{
			...
			while(true)
			{
				if(section == 0)
				{
					...
					//LogAdd(LOG_RED, "Item bag loaded name %s", this->m_EventName);
...
bool CItemBag::DropItem(LPOBJ lpObj,int map,int x,int y) // OK
{
	...
	for(int n=0;n < this->m_ItemDropCount;n++)
	{
		...
		{
			gLog.Output(ITEM_BAG_LOG, "[ItemBag][%s][%s] [Item: %s +%d +%d]", lpObj->Account, lpObj->Name, gItemManager.GetName(item.m_Index), item.m_Level, item.m_Option3);
...
bool CItemBag::GremoryCaseDropItem(LPOBJ lpObj,int map,int x,int y,BYTE btStorageType,BYTE btRewardSource,int iExpireDays) // OK
{
	if(this->m_ItemBagEx.m_ItemBagInfo.empty() == 0)
	{
		return this->m_ItemBagEx.DropItem(lpObj,map,x,y);
	...
	for(int n=0;n < this->m_ItemDropCount;n++)
	{
		...
		if((GetLargeRand()%10000) < this->m_SetItemDropRate)
		{
			gSetItemType.GremoryCaseMakeRandomSetItem(lpObj->Index,map,px,py,btStorageType,btRewardSource,iExpireDays);
			continue;
		}
		...
		if(this->GetItem(lpObj,&item) != 0)
		{
			...
			g_GremoryCase.GDReqAddItemToGremoryCase(lpObj->Index, stItem, iExpireDays);
		}
	}
