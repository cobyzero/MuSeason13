...
void CMove::Move(LPOBJ lpObj,int index) // OK
{
	...
	if(MoveInfo.MinLevel != -1 && (lpObj->Level + lpObj->MasterLevel) < gGate.GetMoveLevel(lpObj,gGate.GetGateMap(MoveInfo.Gate),MoveInfo.MinLevel))
	{
	...
	if(gGate.GetGateMap(MoveInfo.Gate) == MAP_ATLANS && (lpObj->Inventory[8].IsItem() != 0 && (lpObj->Inventory[lpObj->m_btInvenPetPos].m_Index == GET_ITEM(13,2) || lpObj->Inventory[lpObj->m_btInvenPetPos].m_Index == GET_ITEM(13,3)))) // Uniria,Dinorant
	{
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(228));
		return;
	}

	if((gGate.GetGateMap(MoveInfo.Gate) == MAP_ICARUS || gGate.GetGateMap(MoveInfo.Gate) == MAP_KANTURU3) && (lpObj->Inventory[7].IsItem() == 0 && lpObj->Inventory[lpObj->m_btInvenPetPos].m_Index != GET_ITEM(13,3) && 
lpObj->Inventory[lpObj->m_btInvenPetPos].m_Index!= GET_ITEM(13,37)))
	{
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(228));
		return;
	}
	...
	if(CA_MAP_RANGE(gGate.GetGateMap(MoveInfo.Gate)) == 0 || gCustomArena.CheckEnterEnabled(lpObj,MoveInfo.Gate) != 0)
	{
		...
		{
			if(lpObj->m_btInvenPetPos!=-1 &&lpObj->Inventory[lpObj->m_btInvenPetPos].m_JewelOfHarmonyOption ==1)
			{
...
void CMove::CGTeleportRecv(PMSG_TELEPORT_RECV* lpMsg,int aIndex) // OK
{
	...
	if(lpMsg->gate == 0)
	{
		...
		if (gSkillManager.CheckSkillBlock(lpObj, SKILL_TELEPORT))
		{
			return;
		}
...
void CMove::CGTeleportMoveRecv(PMSG_TELEPORT_MOVE_RECV* lpMsg,int aIndex) // OK
{
	...
	//LogAdd(LOG_RED, "Gate %d", lpMsg->number);

	this->Move(&gObj[aIndex],lpMsg->number);
}

void CMove::GCTeleportSend(int aIndex,int gate,WORD map,BYTE x,BYTE y,BYTE dir) // OK
{
...
#
	...
	/*else
	{
		...
		{
			if (g_InstanceSystemMgr->m_cInstanceObject[i].GetOwnerUserIndex()==aIndex)
			{
