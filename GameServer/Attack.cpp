...
#pragma region ATTACK_CHECK
	...
	if (lpObj->Map == MAP_KNICKS && lpObj->Type == 1 && g_NixiesLakeBossZone.IsBossZone(lpTarget->X, lpTarget->Y) == 1 && !g_NixiesLakeBossZone.IsBossZone(lpObj->X, lpObj->Y))
	{
		return 0;
	}
...
#pragma region ATTACK_RETURN
	...
	if ((GetLargeRand() % 100) < gServerInfo.m_BleedingConst)
	{
		if (!gEffectManager.CheckEffect(lpTarget, EFFECT_HEMORRHAGE) && lpObj->PentagramJewelOption.Bleeding)
		{
			int damage = lpObj->PentagramJewelOption.Bleeding;
			gEffectManager.AddEffect(lpTarget, 0, EFFECT_HEMORRHAGE, 10, lpObj->Index, 1, SET_NUMBERHW(damage), SET_NUMBERLW(damage));
		}
	}

	if ((GetLargeRand() % 100) < lpObj->PentagramJewelOption.Paralyzing)
	{
		if (!gEffectManager.CheckEffect(lpTarget, EFFECT_PARALYSIS))
		{
			gEffectManager.AddEffect(lpTarget, 0, EFFECT_PARALYSIS, 20, 0, 0, 0, 0);
		}
	}

	if ((GetLargeRand() % 100) < lpObj->PentagramJewelOption.Binding)
	{
		if (!gEffectManager.CheckEffect(lpTarget, EFFECT_BONDAGE))
		{
			gEffectManager.AddEffect(lpTarget, 0, EFFECT_BONDAGE, 5, 0, 0, 0, 0);
		}
	}

	if ((GetLargeRand() % 100) < gServerInfo.m_BlindConstA)
	{
		if (!gEffectManager.CheckEffect(lpTarget, EFFECT_BLINDNES) && lpObj->PentagramJewelOption.Blinding)
		{
			gEffectManager.AddEffect(lpTarget, 0, EFFECT_BLINDNES, 5, lpObj->PentagramJewelOption.Blinding, 0, 0, 0);
		}
	}
...
#pragma endregion
	...
	int i;
	int MonsterDamageInPlayer = -1;
	int PlayerDamageInMonster = -1;

	CUSTOM_MONSTER_INFO CustomMonsterInfo;

	if (gCustomMonster.GetCustomMonsterInfo(lpObj->Class, lpObj->Map, &CustomMonsterInfo) != 0)
	{
		//atacker is a custom monster			
		if (CustomMonsterInfo.MinDmgMakePerc > -1 || CustomMonsterInfo.MaxDmgMakePerc > -1)
		{
			int MonsterDamageInPlayerFactor = CustomMonsterInfo.MinDmgMakePerc + (rand() % (CustomMonsterInfo.MaxDmgMakePerc - CustomMonsterInfo.MinDmgMakePerc));
			MonsterDamageInPlayer = (lpTarget->MaxLife + lpTarget->AddLife) * MonsterDamageInPlayerFactor / 100;
			if (MonsterDamageInPlayer < CustomMonsterInfo.MinDmgMakePoints)
				MonsterDamageInPlayer = CustomMonsterInfo.MinDmgMakePoints;

			gObjectManager.CharacterLifeCheck(lpObj, lpTarget, MonsterDamageInPlayer, 0, flag, 0, ((lpSkill == 0) ? 0 : lpSkill->m_index), 0);

			return 1;
		}
	}

	if (gCustomMonster.GetCustomMonsterInfo(lpTarget->Class, lpTarget->Map, &CustomMonsterInfo) != 0)
	{
		//target is a custom monster
		if (CustomMonsterInfo.MinDmgGetPoints > -1 || CustomMonsterInfo.MaxDmgGetPoints > -1)
		{
			PlayerDamageInMonster = CustomMonsterInfo.MinDmgGetPoints + (rand() % (CustomMonsterInfo.MaxDmgGetPoints - CustomMonsterInfo.MinDmgGetPoints));
			gObjectManager.CharacterLifeCheck(lpObj, lpTarget, PlayerDamageInMonster, 0, flag, 0, ((lpSkill == 0) ? 0 : lpSkill->m_index), 0);
			return 1;
		}
	}
...
#pragma region DAMAGE_CALC
	...
	if (damage == 0)
	{
		...
		if (lpTarget->ParryRate > 0 && (lpTarget->Inventory[0].m_Index >= GET_ITEM(0, 0) && lpTarget->Inventory[0].m_Index < GET_ITEM(4, 0)))
		{
			float ParryRate = (float)((GetLargeRand() % 10000) / 100.0f);

			if (skill == SKILL_NONE && lpTarget->Type == OBJECT_USER && ParryRate <= lpTarget->ParryRate)
			{
				effect |= 9;
		...
		if (lpTarget->BlockRate > 0 && (lpTarget->Inventory[1].m_Index >= GET_ITEM(6, 0) && lpTarget->Inventory[1].m_Index < GET_ITEM(7, 0)))
		{
			...
			if (BlockRate <= lpTarget->BlockRate)
			{
				effect |= 8;
				GCDamageSend(lpObj->Index, lpTarget->Index, 0, 0, effect, 0);
				return 1;
		...
		else if (skill == SKILL_HARSH_STRIKE || skill == SKILL_BRECHE)
		{
			damage = (damage * lpObj->FKDamageMultiplierRate[0]) / 100;
		}
		else if (skill == SKILL_SPIN_STEP)
		{
			damage = (damage * lpObj->FKDamageMultiplierRate[1]) / 100;
		...
		else if (skill == SKILL_FK_EXPLOSION)
		{
			...
			damage += gMasterSkillTree.GetMasterSkillValue(lpObj, MASTER_SKILL_ADD_FK_SPIN_SETP_MASTERY);
		}
		else if (skill == SKILL_MAGIC_PIN)
		{
			damage = (damage * lpObj->FKDamageMultiplierRate[1]) / 100;
		}
		else if (skill == SKILL_SHINING_PEAK)
		{
			damage = (((damage * lpObj->FKDamageMultiplierRate[0]) / 100) + ((damage * lpObj->FKDamageMultiplierRate[1]) / 100));
		}
		...
		if (count > 0 && (skill == SKILL_UPPER_BEAST || skill == SKILL_DARK_SIDE || skill == SKILL_DARK_PHOENIX_SHOT))
		{
			effect |= (((count % 2) == 0) ? 0x20 : 0x10);
		}

		if (count > 0 && (skill == SKILL_HARSH_STRIKE || skill == SKILL_MAGIC_PIN))
		{
		...
		if ((GetLargeRand() % 100) < ((lpObj->m_MPSkillOpt.DoubleDamageRate) - lpTarget->ResistDoubleDamageRate))
		{
			effect |= 0x40;
			damage += damage;
		}

		if ((GetLargeRand() % 100) < lpObj->m_MPSkillOpt.TripleDamageRate)
		{
			effect |= 0x100;
...
#pragma region DAMAGE_CONFIG
	...
	//General items damage rate
	for (iItemIndex = 0; iItemIndex <= 11; iItemIndex++)
	{
		if (lpObj->Inventory[iItemIndex].IsItem())
		{
	...
	if (lpObj->Type == OBJECT_USER)
	{
		if (lpTarget->Type == OBJECT_USER)
		{
			...
			if (gMUFC.Status == MUFC_STARTED && gMUFC.FightDamage)
			{
				int MUFCdamageBonus;
...
#pragma region APPLY_EFFECT
	...
	if (lpObj->Type == OBJECT_USER && effect != 4)
	{
		...
		if (lpObj->AbsorbSD > 0 && lpObj->Shield < (lpObj->MaxShield + lpObj->m_MPSkillOpt.AddShield) && (GetLargeRand() % 100) < 50)
		{
			lpObj->Shield += (int)lpObj->AbsorbSD;
			GCLifeSend(lpObj->Index, 0xFF, (int)lpObj->Life, lpObj->Shield);
		}

		if ((GetLargeRand() % 100) < lpObj->m_MPSkillOpt.OffensiveFullSDRestoreRate)
		{
	...
	if (lpTarget->Type != OBJECT_USER && lpTarget->Inventory[1].m_Index >= GET_ITEM(6, 29) && lpTarget->Inventory[1].m_Index <= GET_ITEM(6, 32))
	{
		...
		CutBP += gMasterSkillTree.GetMasterSkillValue(lpTarget, MASTER_SKILL_ADD_FK_CIRCLE_SHIELD_POW_UP);
		CutBP += gMasterSkillTree.GetMasterSkillValue(lpTarget, MASTER_SKILL_ADD_FK_CIRCLE_SHIELD_MASTERY);
		int m_rand = 60;
		m_rand += gMasterSkillTree.GetMasterSkillValue(lpTarget, MASTER_SKILL_ADD_FK_CIRCLE_SHIELD_MASTERY);
...
#pragma region ATTACK_FINISH
	...
	if (damage > 0)
	{
		...
		if (effect != 4 && lpTarget->Type == OBJECT_USER)
		{
			...
			else if ((lpTarget->DamageReflect + lpTarget->EffectOption.AddDamageReflect) > 0)
			{
				...
				gObjAddMsgSendDelay(lpTarget, 10, lpObj->Index, 10, ((damage * DamageReflect) / 100));
			}
			...
			if (lpTarget->AbsorbRate > 0 && (lpTarget->Inventory[1].m_Index >= GET_ITEM(6, 0) && lpTarget->Inventory[1].m_Index < GET_ITEM(7, 0)))
			{
				float AbsorbRate = (float)((GetLargeRand() % 10000) / 100.0f);

				if (AbsorbRate <= lpTarget->AbsorbRate)
				{
					int AbsorbedDamage = damage - (10 * lpTarget->Inventory[1].m_DefenseSuccessRate);
					...
					damage -= 10 * lpTarget->Inventory[1].m_DefenseSuccessRate;
				}
			}
			...
			if (/*lpTarget->Inventory[8].IsItem() == 0*/lpTarget->Inventory[lpTarget->m_btInvenPetPos].IsItem() == 0 || (lpTarget->Inventory[lpTarget->m_btInvenPetPos].m_Index != GET_ITEM(13, 2) && lpTarget->Inventory[lpTarget->m_btInvenPetPos].m_Index != GET_ITEM(13, 3) && lpTarget->Inventory[lpTarget->m_btInvenPetPos].m_Index != GET_ITEM(13, 4) && lpTarget->Inventory[lpTarget->m_btInvenPetPos].m_Index != GET_ITEM(13, 37)))
			{
	...
	if (gMUFC.Status == MUFC_STARTED)
	{
		...
		if (gMUFC.FighterID[1] == lpObj->Index)
		{
...
#pragma region DAMAGE_CALC
	...
	if (lpObj->Inventory[7].m_Index == GET_ITEM(12, 270))
	{
		damage = (damage * (139 + (lpObj->Inventory[7].m_Level * 2))) / 100;
	}
...
void CAttack::WingSprite(LPOBJ lpObj, LPOBJ lpTarget, int* damage) // OK
{
	if (lpObj != 0 && lpObj->Type == OBJECT_USER)
	{
		...
		if (lpItem->IsItem() != 0 && lpItem->m_Durability > 0)
		{
			...
			{
				(*damage) = ((*damage) * (171 + (lpItem->m_Level * 1))) / 100;
			}
	...
	if (lpTarget != 0 && lpTarget->Type == OBJECT_USER)
	{
		...
		if (lpItem->IsItem() != 0 && lpItem->m_Durability > 0)
		{
			...
			else if (lpItem->m_Index == GET_ITEM(12, 270) || (lpItem->m_Index >= GET_ITEM(12, 36) && lpItem->m_Index <= GET_ITEM(12, 39)) || lpItem->m_Index == GET_ITEM(12, 43) || lpItem->m_Index == GET_ITEM(12, 50) || // 3rd wing
				(lpItem->m_Index >= GET_ITEM(12, 430) && lpItem->m_Index <= GET_ITEM(12, 437))) // 3rd wing Bound
			{
...
void CAttack::HelperSprite(LPOBJ lpObj, LPOBJ lpTarget, int* damage) // OK
{
	if (lpObj != 0 && lpObj->Type == OBJECT_USER)
	{
		...
		if (lpObj->Inventory[lpObj->m_btInvenPetPos].m_Index == GET_ITEM(13, 3)) // Dinorant
		{
			lpObj->Life -= 1;
		...
		else if (lpObj->Inventory[lpObj->m_btInvenPetPos].m_Index == GET_ITEM(13, 37))
		{
	...
	if (lpTarget != 0 && lpTarget->Type == OBJECT_USER)
	{
		...
		{
			(*damage) = ((*damage) * (100 - ((gServerInfo.m_DarkHorseDecDamageConstA + lpTarget->Inventory[lpTarget->m_btInvenPetPos].m_PetItemLevel) / gServerInfo.m_DarkHorseDecDamageConstB))) / 100;
		}
		...
		{
			if ((lpTarget->Inventory[lpTarget->m_btInvenPetPos].m_NewOption & 2) != 0)
			{
...
{
	CItem* lpItem = &lpObj->Inventory[lpObj->m_btInvenPetPos]; //&lpObj->Inventory[8];
...
void CAttack::MissSend(LPOBJ lpObj, LPOBJ lpTarget, CSkill* lpSkill, int send, int count) // OK
{
	...
	if (count > 0 && lpSkill != 0 && (lpSkill->m_skill == SKILL_UPPER_BEAST || lpSkill->m_skill == SKILL_DARK_SIDE || lpSkill->m_skill == SKILL_DARK_PHOENIX_SHOT))
	{
		effect |= (((count % 2) == 0) ? 0x20 : 0x10);
	}
...
#if(GAMESERVER_UPDATE>=1000)//2017/05/01
...
#if(GAMESERVER_UPDATE>=1100)//2017/04/25
	if (lpObj->Type == OBJECT_USER)
	{
...
bool CAttack::ApplySkillEffect(LPOBJ lpObj, LPOBJ lpTarget, CSkill* lpSkill, int damage) // OK
{
	...
	case SKILL_POISON_ARROW:
		gEffectManager.AddEffect(lpTarget, 0, gSkillManager.GetSkillEffect(lpSkill->m_index), 10, lpObj->Index, 1, (lpObj->PhysiDamageMinLeft / 5), 0);
		break;
...
int CAttack::GetTargetDefense(LPOBJ lpObj, LPOBJ lpTarget, WORD* effect) // OK
{
	...
	if ((GetLargeRand() % 100) < ((lpObj->m_MPSkillOpt.IgnoreDefenseRate + lpObj->EffectOption.AddIgnoreDefenseRate) - lpTarget->ResistIgnoreDefenseRate))
	{
		(*effect) = 1;
		defense = 0;
...
int CAttack::GetTargetElementalDefense(LPOBJ lpObj, LPOBJ lpTarget, WORD* effect) // OK
{
	...
	if (lpTarget->Type == OBJECT_USER)
	{
#if(GAMESERVER_UPDATE>=1000)//2017/04/25
...
#if(GAMESERVER_UPDATE>=1100)//2017/04/23��ӡ
...
int CAttack::GetAttackDamageWizard(LPOBJ lpObj, LPOBJ lpTarget, CSkill* lpSkill, WORD* effect, int TargetDefense) // OK
{
...
#if(GAMESERVER_UPDATE>=1120)//2017/05/11
	int CriticalDamageRate = (((lpObj->m_MPSkillOpt.CriticalDamageRate + lpObj->EffectOption.AddCriticalDamageRate) > gServerInfo.m_CriticalDamageRate) ? gServerInfo.m_CriticalDamageRate : (lpObj->m_MPSkillOpt.CriticalDamageRate + lpObj->EffectOption.AddCriticalDamageRate));
	if ((GetLargeRand() % 100) < (CriticalDamageRate - lpTarget->ResistCriticalDamageRate))
#else
	if ((GetLargeRand() % 100) < ((lpObj->m_MPSkillOpt.CriticalDamageRate + lpObj->EffectOption.AddCriticalDamageRate) - lpTarget->ResistCriticalDamageRate))
#endif
	{
		(*effect) = 3;
	...
	if ((GetLargeRand() % 100) < (ExcellentDamageRate - lpTarget->ResistExcellentDamageRate))
#else
	if ((GetLargeRand() % 100) < ((lpObj->m_MPSkillOpt.ExcellentDamageRate + lpObj->EffectOption.AddExcellentDamageRate) - lpTarget->ResistExcellentDamageRate))
#endif
	{
		(*effect) = 2;
...
/*int CAttack::GetAttackDamageElemental(LPOBJ lpObj, LPOBJ lpTarget, CSkill* lpSkill, WORD* effect, int AttackDamage, int TargetDefense) // OK
{
	...
	if (lpTarget->Type == OBJECT_USER)
	{
		if (lpTarget->Shield <= gServerInfo.m_BastionConstA)
		{
			if ((GetLargeRand() % 100) < gServerInfo.m_BastionConstB && !gEffectManager.CheckEffect(lpTarget, EFFECT_BASTION))
			{
				if (lpTarget->PentagramJewelOption.Bastion)
				{
					gEffectManager.AddEffect(lpTarget, 0, EFFECT_BASTION, lpTarget->PentagramJewelOption.Bastion, 0, 0, 0, 0);
	...
	if ((GetLargeRand() % 100) < (lpObj->m_MPSkillOpt.CriticalDamageRate + lpObj->EffectOption.AddCriticalDamageRate) - lpTarget->ResistCriticalDamageRate)
	{
		...
		damage = DamageMax;
		if (((GetLargeRand() % 100) < gServerInfo.m_PunishConst) && lpObj->PentagramJewelOption.Punish)
		{
			damage = ((int)(lpTarget->MaxLife) * lpObj->PentagramJewelOption.Punish) / 100;
		}
	}
...
void CAttack::GetPreviewDamageMultiplier(LPOBJ lpObj, DWORD* DamageMultiplier, DWORD* RFDamageMultiplierA, DWORD* RFDamageMultiplierB, DWORD* RFDamageMultiplierC) // OK
{
	...
	case CLASS_GL:
		...
		(*RFDamageMultiplierA) = lpObj->FKDamageMultiplierRate[0];
		(*RFDamageMultiplierB) = lpObj->FKDamageMultiplierRate[1];
...
void CAttack::CGAttackRecv(PMSG_ATTACK_RECV* lpMsg, int aIndex) // OK
{
	...
	/*if (gCheatGuard.CheckSpeedHack(aIndex, bIndex, 0, "CGAttack") == 0)
	{
		return;
	}*/
