...
public:
	...
	void MountSprite(LPOBJ lpObj, int damage);
	...
	void CGAttack(int aIndex, int bIndex, int action, int dir);
	#if(GAMESERVER_UPDATE>=1100)//2017/04/23©лс║
	int GetAttEngraving(LPOBJ lpObj,CSkill* lpSkill,int damage);
#endif
};
