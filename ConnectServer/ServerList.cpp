	...
	if(this->CheckJoinServerState() != 0)
	{
		for(std::map<int,SERVER_LIST_INFO>::iterator it=this->m_ServerListInfo.begin();it != this->m_ServerListInfo.end();it++)
		{
			if(it->second.ServerShow != 0 && it->second.ServerState != 0)
			{
				...
				if (it->second.IsPKServer == 0 && it->second.IsGoldServer == 1)
				{
					info.type = 2;
				}
				else if (it->second.IsPKServer == 0 && it->second.IsGoldServer == 0)
				{
					info.type = 0;
				}
				else if (it->second.IsPKServer == 1 && it->second.IsGoldServer == 0)
				{
					info.type = 1;
				}
				else if (it->second.IsPKServer == 1 && it->second.IsGoldServer == 1)
				{
					info.type = 3;
				}
...
void CServerList::GCGameServerLiveRecv(SDHP_GAME_SERVER_LIVE_RECV* lpMsg) // OK
{
	...
	lpServerListInfo->IsPKServer = lpMsg->IsPKServer;
	lpServerListInfo->IsGoldServer = lpMsg->IsGoldServer;
}
