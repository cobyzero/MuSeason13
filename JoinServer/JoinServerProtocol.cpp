...
void GJConnectAccountRecv(SDHP_CONNECT_ACCOUNT_RECV* lpMsg,int index) // OK
{
	...
	if(MD5Encryption == 0)
	{
		if(gQueryManager.ExecQuery("SELECT memb__pwd FROM MEMB_INFO WHERE memb___id='%s'",lpMsg->account) == 0 || gQueryManager.Fetch() == SQL_NO_DATA)
		{
			gQueryManager.Close();
			//char szQuery[512];
			/*if (AutoRegistration == TRUE && strlen(lpMsg->account) >= AutoRegistrationMinLength && strlen(lpMsg->password) >= AutoRegistrationMinLength)
			{

				if (gQueryManager.ExecQuery("INSERT INTO MEMB_INFO (memb___id,memb__pwd,memb_name,bloc_code,ctl1_code) VALUES ('%s','%s','lst',0,0)",
					lpMsg->account, lpMsg->password) == FALSE)
				{
					LogAdd(LOG_BLACK, "error-L3 : ID:'%s' PASS:'%s'", lpMsg->account, lpMsg->password);
					gQueryManager.Close();
				
				}
				else
				{
					LogAdd(LOG_BLACK, "[CreatAccount]: ID:'%s' PASS:'%s'", lpMsg->account, lpMsg->password);
					gQueryManager.Close();
				}
			}*/
	...
	else
	{
		if(gQueryManager.ExecQuery("SELECT memb__pwd FROM MEMB_INFO WHERE memb___id='%s'",lpMsg->account) == 0 || gQueryManager.Fetch() == SQL_NO_DATA)
		{
			/*if (AutoRegistration == TRUE && strlen(lpMsg->account) >= AutoRegistrationMinLength && strlen(lpMsg->password) >= AutoRegistrationMinLength)
			{
				if (gQueryManager.ExecQuery("INSERT INTO MEMB_INFO (memb___id,memb__pwd,memb_name,bloc_code,ctl1_code) VALUES ('%s',dbo.fn_md5('%s','%s'),'lst',0,0)", lpMsg->account, lpMsg->password, lpMsg->account) == FALSE)
				{
					LogAdd(LOG_BLACK, "error-L3 : ID:'%s' PASS:'%s'", lpMsg->account, lpMsg->password);
					gQueryManager.Close();
				}
